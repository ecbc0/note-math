...
where $Γ$ is #tag("metric_connection") *alias* #tag("Levi_Civita_connection") *alias* #tag("Christoffel_symbols")
#indent[
  $
    && v^⊺ Γ v
    ...
    &= 1/2 g^(-1) ((∂ (v') g) v + (∂ (v) g) v' - v'^⊺ (∂ g) v) 
      &quad ("cf. " #link(<difference_symmetric_tensor>)[difference]) \

    "or "&& Gamma_(i i')^j 
    &= 1/2 sum_i'' g^(j i'') (∂_i g_(i' i'') + ∂_i' g_(i i'') - ∂_i'' g_(i i'))
  $
  metric-connection is not a tensor. The transformation of metric-connection #tag("connection_transformations")
  $
    Γ(y) 
...
#tag("geodesic_exponential") ...

#tag("geodesic_coordinate") 
#indent[
...
#tag("Taylor_expansion_of_metric_in_geodesic_coordinate") 
#indent[
...
#indent[
  ...
  - 1st. 
  #indent[
    #tag("differenial_of_metric_vs_connection") *Prop* $v^⊺ (∂(v'') g) v' = v'^⊺ g (v''^⊺ Γ v) + v'^⊺ g (v''^⊺ Γ v)$ _Proof_ Directly substitute the #link(<metric_connection>)[definition] of $Γ$ into the formula
...
#tag("differenial_of_metric_inverse_vs_connection") *Prop* $α^⊺ (∂(α'') g^(-1)) α' = - ( α'^⊺ g^(-1) (α''^⊺ Γ α) + α'^⊺ g^(-1) (α''^⊺ Γ α) )$
