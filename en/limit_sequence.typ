...
#tag("sequence_real") Real sequence := $ℕ -> ℝ$. Usually denoted as $a_n$. Depending on the situation, start from $0$ or $1$

#tag("limit_sequence_real") Limit of sequence $a_n$
...
#tag("rational_dense_in_real") Density of $ℚ$ in $ℝ$. $forall x,y ∈ ℝ, exists m/n ∈ ℚ, x < m/n < y$

_Proof_
#indent[
  $x < y$ #link(<order_real>)[is equivalent to]
...
#tag("geometric_series") Geometric series $x_n = 1 + a + a^2 + ⋯ + a^n$. $0 < a < 1 ==> lim_(n ->  ∞) x_n = 1/(1 - a)$

_Proof_ $(1 - a)(1 + a + a^2 + ⋯ + a^n) = 1 - a^(n + 1)$, $x_n = (1 - a^(n + 1))/(1 - a)$

#tag("geometric_series_test") Geometric series convergence test. let $a_n > 0$. $a_(n + 1)/a_n < q < 1 ==> lim_(n ->  ∞) a_n = 0$

_Proof_ $a_n = a_n/a_(n - 1) ⋯ a_0/a_1 a_0 ⋯  < q^n a_0$

#tag("exponential_vs_power") Exponential growth is faster than power. $a > 1 , p ∈ ℕ ==> lim_(n -> ∞) n^p/a^n = 0$

_Proof_ define $b_n = n^p/a_n$

  use Geometric series convergence test $b_(n + 1)/b_n = ((n + 1)/n)^p 1/a -->_(n -> ∞) 1/a < 1$

#tag("exponential_root_of_power_function") $lim_(n -> ∞) n^(1/n) = 1$
...
#tag("factorial_vs_exponential_1") Factorial grows faster than exponential. $a ∈ ℝ ==> lim_(n -> ∞) a^n/n! = 0$
...
#tag("iterated_power_vs_factorial") $lim_(n -> ∞) n!/n^n = 0$
...
#tag("mean_inequality") Mean inequality *alias* #tag("AM_GM_inequality")
#indent[
...
#tag("best_multiplication_decomposition") optimal multiplication decomposition
#indent[
...
#tag("natural_constant") Natural constant $e$
...
#indent[
  ...
  $
    ...
    &= (z + 1) ⋯ (z/n + 1)
  $
  Use #link(<Taylor_expansion>)[Taylor expansion] $log (1 + z/n) = z/n + O(z/n)^2 $
  $
  ...
  - Using the properties of the factorial function, we can prove $sum_(n = 1 .. ∞) 1/n^2 = (π^2)/6$ cf. #link(<Euler_reflection_formula>)[]. Here we only prove convergence
  ...
  - $lim_(n -> ∞) 1 + 1/2 + ⋯ + 1/n - log n = γ$ is the #link(<Euler_constant>)[Euler gamma constant] #tag("Euler_constant")
  #indent[
...
#tag("Euler_reflection_formula") Euler reflection formula $1/(z! (-z)!) = product_(n = 1 .. ∞) (1 - z^2/n^2) = (sin π z)/(π z)$ or $z/((z/π)! (-z/π)!) = z product_(n = 1 .. ∞) (1 - z^2/n^2) = sin z$
#indent[
...
$
  ...
  &= π
$
#tag("iterated_power_vs_factorial_2")
#indent[
...
#indent[
  ...
  #tag("sequence_multiplication_mean_limit") Multiplication mean does not change the limit
...
#tag("sequence_addition_mean_limit") Addition mean $lim_(n -> ∞) a_n = a ==> lim_(n -> ∞) 1/n (a_1 + ⋯ + a_n) = a$

#tag("harmonic_series_diverge") Harmonic series diverges $lim_(n -> ∞) 1 + 1/2 + ⋯ + 1/n = +∞$

_Proof_ $a_n = 1 + 1/2 + ⋯ + 1/n$ diverges by it is not #link(<Cauchy_completeness_real>)[limit-distance-vanish]. e.g.

  $ forall n ∈ ℕ, a_(2n) - a_n = 1/(n+1) + ⋯ + 1/(2n) >= n ⋅ 1/(2n) = 1/2 $

#tag("iterated_power_vs_factorial_3") #tag("Stirling_approximation") $n! ∼ (2 π n)^(1/2) n^n/(e^n)$
#indent[
  Usage Tips
  $
  ...
  Discussions on the appearance of $π$ can also be seen in #link(<why_pi_in_Gaussian_integral>)[]
]