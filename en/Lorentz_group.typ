...
#tag("isotropy_on_lightcone") *Prop* $SO(1,3)$ acting on lightcone is similar to $SO(2) ⋊ ℝ^2$ (exactly the $ℝ^2$ Euclidean affine group)

_Proof_ Using #link(<spacetime_momentum_aciton_spinor_representation>)[spinor technology]
#indent[
  *Prop* $SL(2,ℂ)$ acting on lightcone (not projective-lightcone), #link(<isotropy>)[] is similar to $#U (1) ⋊ ℂ$

  $SL(2,ℂ)$ is #link(<action_surjective>)[surjective action], orbit number $1$, so calculate isotropy #link(<isotropy_in_same_orbit_is_isom>)[only need to consider] one point
  ...
  Similar to the calculation of #link(<isotropy_on_projective_lightcone>)[] , here it will be similar to $#U (1) ⋊ ℂ$
]

#tag("isotropy_on_lightcone_intuition") Intuition of the isotropy-group of orbit lightcone. According to
#indent[
  $
