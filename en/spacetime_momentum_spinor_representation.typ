...
#indent[
  ...
  Its complex conjugate #tag("conjugate_representation")
...
#tag("Hermitian_tensor_induced_linear_map") The induced action of $A ∈ GL(2,ℂ)$ on $⨂^(* 2) ℂ^2$ :=
...
#tag("Hermitian_tensor_induced_linear_map_matrix") The matrix representation of $A^(⊗ * 2)$
#indent[
  $ v ⋅ w^† ⇝ (A v) ⋅ (A w)^† = A (v ⋅ w^†) A^† $
...
- $SO(1,3) ≃ SL(2,ℂ)/ℤ_2$ #link(<Lorentz_group_spinor_representation>)[acts on] $ℂℙ^1$ lifts to $SL(2,ℂ)$ act on $ℂ^2$
#indent[
...
#tag("square_root_of_spacetime_metric_1") (inspired by @ref-14, ch.11)
#indent[
...
#indent[
  ...
  where $f(A)$ is #link(<Lorentz_group_spinor_representation>)[]
  
  $f(#p) = p$ is #link(<spacetime_momentum_spinor_representation>)[] 
...
*Prop* Use #link(<spacetime_momentum_spinor_representation>)[] for $v ⋅ w^†$, $v = w$ + $ℂ$ projection $λ v$ gives projective-lightcone
$
...
#tag("parity") 
#indent[  
  parity corresponds to $(1/2,0)$ vs $(0,1/2)$ representation, or $A$ vs $A^*,(A^†)^(-1)$, cf. #link(<conjugate_representation>)[]
...
#tag("square_root_of_spacetime_metric_2") a "square root" of $1,3$ metric 
#indent[
...
#tag("square_root_of_Lorentz_Lie_algebra") "square root" of spacetime Lie-algebra
#indent[
...
#tag("parity_Euclidean_invariant") parity commutes with spatial action $SU(2)$. In $ℝ^3$, it manifests as $- 𝟙$ and commutes with $SO(3)$. let $p ∈ ℝ^3, A ∈ SU(2)$
...
#tag("parity_reverse_boost") The effect of parity on the Lie-algebra is that it does not change δ rotation, but multiplies δ boost by $-1$
