...
Using the alternation of #link(<tensor_induced_quadratic_form>)[]
$
...
#tag("quadratic_form_inequality_Euclidean") Inner product inequality (Euclidean). $0 <= ⟨v ∧ w⟩^2 = det mat(⟨v⟩^2, ⟨v,w⟩; ⟨w,v⟩, ⟨w⟩^2) = ⟨v⟩^2 ⟨w⟩^2 - ⟨w,v⟩^2$. i.e. $⟨w,v⟩^2 <= ⟨v⟩^2 ⟨w⟩^2$ or $⟨v,w⟩ <= |v| |w|$

#tag("triangle_inequality_Euclidean") Triangle inequality (Euclidean)
  - $|v + w| <= |v| + |w|$
...
#tag("Euclidean_space_topology") Euclidean $ℝ^d$ topology. $f : ℝ^d -> ℝ^d'$ is continuous at $a ∈ ℝ^d$ :=
  $
...
#tag("closed_set") Closed set := $closed(A) = A$

(open) ball $𝔹(a,r) := {x ∈ ℝ^d : |x - a| < r}$

#tag("open_set") Open set $U ⊆ ℝ^d$ := $forall x ∈ U, exists r > 0, 𝔹(x,r) ⊆ U$
...
$
#tag("best_interval_decomposition") Optimal interval decomposition of $A ⊆ ℝ$
#indent[
  ...
  *Def* $#J (A) := {I ⊆ A : I ∈ "Interval"}$, i.e., the set of all intervals among the subsets of $A$
  ...
  $#J (A)$ has $⊆$ #link(<linear_order>)[linear order chains]. Taking $⋃$ of each maximal linear order chain yields an interval. The set of these intervals is denoted $#I (A)$
...
#tag("bounded_closed_interval_is_compact") A bounded closed interval of $ℝ$ ==> #link(<compact>)[]
...
#tag("compact_imply_subsequence_converge") $A$ compact ==> sequence ${x_n} ⊆ A$ has a convergent subsequence. Similarly for nets
...
#indent[
  $𝕊^1$ is continuously isomorphic to $ℝ/ℤ$ (#link(<quotient_topology>)[]) is continuously isomorphic to $closed(𝔹)^1/𝕊^0$ i.e. $[-1,1] = closed(𝔹)^1$ collapsing endpoints ${-1,1} = 𝕊^0$ (quotient-topology)

  $closed(𝔹)^1 = [-1,1]$ is a bounded closed interval, hence compact ==> the quotient $𝕊^1 = closed(𝔹)^1/𝕊^0$ is compact. by #link(<quotient_topology_preserve_compact>)[quotient preserves compactness]
]
...
#tag("Euclidean_space_compact_iff_bounded_closed") $A ⊆ ℝ^n$ compact <==> $A$ is bounded and closed
...
#indent[
  ...
  #indent[
    ...
    - bounded
    #indent[
      $ℝ^d$ open ball is away from $∞$. The family of open balls ${𝔹(x,r) ⊆ ℝ^n : (x ∈ A) and (r > 0)}$ covers $A$. Take #link(<compact_finite_open_cover>)[finite cover], finite union remains away from $∞$, thus $A$ is also away from $∞$, i.e., $A$ is bounded
    ]
  ]
]
#tag("nested_closed_set_theorem") The intersection of a nested sequence of bounded closed sets in $ℝ^n$ is nonempty. Its intersection is also a closed set, which can be understood as the minimal element of the ⊆ linearly ordered chain of nested closed sets

#tag("closed_net_theorem") The intersection of a net of bounded closed sets in $ℝ^n$ is nonempty
_Proof_
#indent[
...
Some infinite-dimensional linear spaces e.g. #link(<Lebesgue_integrable>)[] $L^1$, bounded closed sets do not imply compact, but still satisfy that a net with vanishing limit-distance converges to a point, due to the completeness of $L^1$
...
#tag("series_rearrangement_absolutely_convergence_real") *Prop* Absolute convergence $sum_(.. ∞) |a_n|$ ==> $sum_(.. ∞) a_n$ converges and is invariant under rearrangement
...
_Proof_
#indent[
  - $sum a_n$ converges. By using the triangle inequality $abs(sum_(N+1)^∞ a_n) <= sum_(N+1)^∞ |a_n|$ and the #link(<Cauchy_completeness_Euclidean>)[Cauchy sequence convergence] in $ℝ^d$
