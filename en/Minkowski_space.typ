...
#tag("signature_of_2d_subspace_of_spacetime") *Prop* The possible signatures of a $dim = 2$ $span(v,w)$ in Minkowski $(1,n)$ are
...
- light-like vectors other than those colinear with itself $k v$ #tag("metric_cannot_distinguish_colinear_light_like")
...
#tag("simultaneity_relativity") Relativity of Simultaneity
#indent[
...
#link(<quadratic_form_inequality_Euclidean>)[Inner product inequality] $|⟨ v,w ⟩|^2 <= abs(v) abs(w)$ ==> #link(<triangle_inequality_Euclidean>)[Triangle inequality] $|v+w| <= |v|+|w|$
...
#tag("quadratic_form_inequality_Minkowski") Inner product inequality
- $⟨ v ⟩^2 ⟨ w ⟩^2 < ⟨ v,w ⟩^2$ \
...
#tag("triangel_inequality_Minkowski") $1,1$ signature triangle inequality
#indent[
  $⟨ v + w ⟩^2 = ⟨ v ⟩^2 + 2 ⟨ v,w ⟩ + ⟨ w ⟩^2$
...
#indent[
  ...
  #indent[
    ...
    use #link(<quadratic_form_inequality_Minkowski>)[] $⟨ v ⟩^2 ⟨ w ⟩^2 < ⟨ v,w ⟩^2$
...
#image("../image/hyperbolic_to_space_bijective.png", width: 50%)
...
$"continuous"(Q -> Q)$ may map lightcone to lightcone. It's said that Zeeman prove that in Minkowski space with spatial dimension larger than $1$, the (bijective map) isomorphism group that preserve any point on lightcone and time order between two point on lightcone is generated by quadratic rotation + translation + scale

Possible clue for the rationality of the $p, q$ quadratic form distance: $ℝ^(p, q)$, via the $(1, 1)$ tensor $ℝ^(p,q)^⊺ ⊗ ℝ^(p,q)$ as a linear map space, inherits a tensor quadratic form. When restricted to $so(p,q)$, it becomes proportional to the Killing form quadratic form of $so(p, q)$ #link(<Killing_form_of_orthogonal_group>)[]. Signature $(p q, binom(p,2) + binom(q, 2))$, where $p q$ is the number of boosts, $binom(p,2)$ are time-like rotations, and $binom(q,2)$ are space-like rotations. Under the Killing form, boosts have positive distance, while time/space rotations both have negative distance.
...
Similar to polar coordinates in $ℝ^2$, we can use hyperbolic polar coordinates $(r, ϕ)$, where $r$ is the quadratic form distance, and $ϕ$ is the geodesic length on $ℍ𝕪$, also called the hyperbolic angle #tag("hyperbolic_angle") or rapidity #tag("rapidity")
...
#tag("polor_coordinate_hyperbolic") Hyperbolic polar coordinates $ℝ^(1,1)(time,future) ≃ ℝ(>=0) × ℍ𝕪$
$
  mat(delim: #none,
...
$
#tag("hyperbolic_cosine_formula") Hyperbolic Cosine Formula
#indent[
  let $v,w in ℍ𝕪$
...
#tag("isom_top_hyperbolic_Euclidean")
...
$ℍ𝕪^n (time) = ℚ^(1,n)(1)$ type (p. 19 of @ref-9) #image("../image/quadratic_manifold_geodesic.png", width: 30%)

$ℍ𝕪^n (space) = ℚ^(1,n)(-1)$ type #stack( dir: ltr,
  image("../image/quadratic_manifold_geodesic_1.png", width: 30%),
  image("../image/quadratic_manifold_geodesic_2.png", width: 30%),
  image("../image/quadratic_manifold_geodesic_3.png", width: 30%)
)
...
#tag("stereographic_projective_hyperbolic") For the time-like hyperboloid $ℍ𝕪^n (time)$, consider stereographic projection. The two base points lie on the two sheets of the hyperboloid respectively, and the projection forms separated singularities in the light cone direction (the cross shape in the figure below)

#image("../image/stereographic_projective_hyperbolic_time_like.jpg", width: 50%)
...
#image("../image/stereographic_projective_hyperbolic_space_like.png", width: 50%)
