...
#indent[
  $ϕ$ induces the co-vector of #link(<Hermitian_tensor>)[]
  $
    map(⨀^(* 2) ℂ^2, ℝ, #p, ϕ^† #p ϕ)
  $
...
#indent[
  - formally corresponds momentum $p$ to gradient momentum $#i ⋅ (∂_0,-∂_1,-∂_2,-∂_3) = #i ⋅ (∂_0,∂_1,∂_2,∂_3)^◊$, and $#p$ to #link(<spacetime_momentum_spinor_representation>)[] 
    $
      #i ⋅ mat(
        ∂_0 + (-∂_1), -(∂_2 + ∂_3) #i ;
        -(∂_2 - ∂_3) #i, ∂_0 - (-∂_1)
...
#tag("massive_spinor_Lagrangian") action for mass-coupled spinors, *alias* #tag("Dirac_Lagrangian")
#indent[
...
#indent[
  ...
  Similar to #link(<linear_superposition_of_KG_eq>)[the scalar field case], $L^2$ superposition can be performed on the hyperboloid $ℍ𝕪^3$ where the momentum lies. #tag("linear_superposition_of_Dirac_eq")
  $
...
#indent[
  ...
  Changing the gauge is not compatible with taking derivatives of tangent spaces in bundle coordinates, so an additional structure --- #link(<principal_bundle_connection>)[connection] --- must be introduced.
  
  There are many possible connections. A good connection is one with the smallest curvature cf. #link(<electromagnetic_field>)[]

  The $ℂ^2$ bundle in curved spacetime can be directly defined in the bundle coordinates of the $SO(1,3)$ principal bundle (orthonormal frame bundle). Using the #link(<Lorentz_group_spinor_representation>)[$SL(1,ℂ) ↠ SO(1,3)$ correspondence], changing $SO(1,3)$ bundle coordinates automatically corresponds to changing $SL(2,ℂ)$ bundle coordinates.

  In curved spacetime, one needs to deal with the covariant derivative of the spinor field with respect to the metric, which is derived from the #link(<metric_connection>)[] of the tangent vector field. 
...
#tag("spin_connection")
#indent[
  The #link(<principal_bundle>)[frame bundle] of $SO(1,3)$ derived from the tangent bundle metric and the #link(<principal_bundle_connection>)[connection] of the $SO(1,3)$ frame bundle derived from #link(<metric_connection>)[] behave as $Γ$ is locally type $(ℝ^(1,3))^⊺ ⊗ so(1,3)$, acting on $ℝ^(1,3)$ tangent vector fields by $∂ + Γ$

  The way to derive the spin-connection is to #link(<square_root_of_Lorentz_Lie_algebra>)[map the $so(1,3)$ part of the induced metric-connection $Γ$ to $sl(2,ℂ)$] in the orthonormal-frame, yielding the connection of the $SL(2,ℂ)$ bundle, locally type $(ℝ^(1,3))^⊺ ⊗ ℂ^2$, acting on the spinor field $ℂ^2$ by $∂ + ω$ with $ω = ω^(μ ν) 1/4 [σ_(μ),σ_(ν)]_(◊)$
