...
#tag("action_point_particle_non_relativity") Action of path $t ⇝ x(t)$ 
...
$
holds for all δ diffeomorphism $X(t)$, thus giving the Lagrange-equation (*alias* Euler--Lagrange-equation), for non-relativistic point particles, #tag("Newton_equation") 
...
#tag("point_particle_Lagrange_equation") 
#indent[
  ...
  $
  is zero at the boundary + integral is zero for all δ diffeomorphism $X$ ==> #tag("point_particle_Lagrange_equation")
  $
    (∂ L)/(∂ x) - (#d)/(#d t) (∂ L)/(∂ vel(x)) = 0
  $
]
...
#indent[  
  ...
  δ variation of the integral area #tag("calculation_1_action_point_particle_non_relativity")
  $ 
  ...
  In general, changing the region $I$ by $exp(a(t))$ #link(<vector_field_as_δ_diffeomorphism>)[by δ diffeomorphism] $a(t)$
  $
    (#d)/(#d s) limits(integral)_(exp(s a(t)) I) #d t (f(t)) 
    
    = integral_(I) #d t ((#d)/(#d t) f(t) ⋅ a(t))
  $
  On the other side, use #link(<integral_change_of_variable_formula>)[change of variable formula] 
  $ 
  ...
  is the variation of the action on the (changing endpoints) δ differentiation $(#d)/(#d s)(0) x(t + s a) = a vel(x)(t) = X(t)$ at the solution $x(t)$ #tag("calculation_2_action_point_particle_non_relativity")
  $
    (#d)/(#d s) integral_(t_0 + s a)^(t_1 + s a) #d t (L(t))
    &= integral #d t ((∂ L)/(∂ x) ⋅ X + (∂ L)/(∂ vel(x)) ⋅ vel(X)) \
  ...
  #tag("energy_point_particle_non_relativity") 
  $
...
#indent[
  ...
  $ 
    ...
    &= 0
  $
  So similar to #link(<calculation_2_action_point_particle_non_relativity>)[the case of energy], with δ diffeomorphism $(#d)/(#d s) (x + s a) = a = X(t)$
  $
    0 = integral_(t_0)^(t_1) #d t ((#d)/(#d t) ((∂ L)/(∂ vel(x)) ⋅ X)) = (∂ L)/(∂ vel(x)) |_(t_0)^(t_1) ⋅ a
  $
  #tag("momentum_point_particle_non_relativity") The momentum of the action $integral #d t (1/2 m vel(x)^2)$ 
...
#indent[
  ...
  #tag("rotation_momentum_point_particle_non_relativity") Rotation momentum rotation-momentum *alias* angular momentum angular-momentum 
  ...
  $a ⋅ (b × c)$ = #link(<volume_of_parallelogram>)[parallelepiped directed volume] span by $a,b,c$ in Euclidean $ℝ^3$
