...
for #link(<Klein__Gordon_Lagrangian>)[], restore Planck constant $ℏ$, speed of light $c$, time $x_0 = c t$
$
...
Non-relativistic scalar field Lagrangian, *alias* Schrodinger-Lagrangian #tag("Schrodinger_Lagrangian")
$
...
Non-relativistic scalar field equation, *alias* Schrodinger-equation #tag("Schrodinger_equation")
$
...
$
Similar to the case of relativistic scalar field, energy-momentum tensor #tag("energy_momentum_tensor_Schrodinger")
$
  T = T^(μ)_(ν) = cases(
...
Rotational angular momentum is #tag("angular_momentum_Schrodinger")
$
  M^k_(i j) = [x_i,T^k_j]
$
...
The entire current, after dividing by $#i ℏ$, is $(ψ^* ψ, - (#i ℏ)/(2m) (ψ^* ∂_x ψ - ψ ∂_x ψ^*)$ #tag("current_gauge_Schrodinger")
...
time-invariant $∂_0 integral_(ℝ^3) #d x ( j^0 ) = 0$ by $∂^† T = 0$ #tag("conserved_spatial_integral_charge_Schrodinger")
$ 
...
#indent[
  ...
  Note the difference between classical energy and quantum energy, similar to the difference between mean and variance. For example, $⟨ #p ⟩^2 != ⟨ #p^2 ⟩$. We can consider the standard deviation $Δ #p = (⟨ #p^2 ⟩ - ⟨ #p ⟩^2)^(1/2)$. There is the uncertainty principle $1/2 ℏ = 1/2 |#i [#x,#p]| <= Δ #x ⋅ Δ #p$. The equality holds <==> #link(<harmonic_oscillator_ground_state>)[]
