...
$
Since the metric is zero on the light cone, many analysis cannot be done. Also, the quotient of rays on the light cone corresponds to #link(<metric_cannot_distinguish_colinear_light_like>)[]
...
#tag("complex_struct_of_4d_projective_lightcone") Complex structure of 4d projective-lightcone (Figure)
  ...
  - Hyperbolic $#projective-cone (1,3) ∖ #projective-cone (1,2) ≃ ℍ𝕪^2$ \ 
    ...
    Is there a $ℂℙ^1$ analogue? But $ℍ𝕪^2$ is a Euclidean manifold, which is not suitable for the $1,1$ signature of split complex numbers $ℂ_"split"$, and #link(<stereographic_projective_hyperbolic>)[stereographic projective hyperbolic projection] seems quite complicated \
...
#indent[
  ...
  - $𝕊^2 ≃ ℂℙ^1$
  #indent[
    #link(<stereographic_projection>)[Stereographic projection] transition-function is quadratic inversion
    ...
    A more direct mapping between the coordinates of $ℂℙ^1 -> 𝕊^2$, cf. #link(<Hopf_bundle>)[] 
  ]
]
#tag("linear_fractional")  
#indent[
...
- $SO(1,3)$ acts on $ℂℙ^1$ in coordinate can be expressed as $SL(2,ℂ)$ #link(<linear_fractional>)[] 

- $SL(2,ℂ)/ℤ_2 = SO(1,3)$ #tag("Lorentz_group_spinor_representation") 
...
#indent[
  ...
  #tag("rotation_boost_spinor_representation")
  ...
  - from $SO(1,3)$ to $SL(2,ℂ)$. Solve it from the following $SL(2,ℂ)$ to $SO(1,3)$. Or use $SO(1,3)$ #link(<polar_decomposition_of_Lorentz_group>)[Polar decomposition] to rotation boost + #link(<Euler_angle_Lorentz_group>)[Euler angle] 

  - from $A = mat(a,b;c,d) in SL(2,ℂ)$ to $Λ in SO(1,3)$, where $a d - b c = 1$ 
  #indent[
    Use #link(<spacetime_momentum_spinor_representation>)[] to directly calculate $A #spin($p$) A^† in ℝ^(1,3)$  
    $ 
...
#tag("isotropy_on_projective_lightcone") *Prop* $SL(2,ℂ)$ acts on projective-lightcone $ℂℙ^1$, #link(<isotropy>)[] is similar to $GL(1,ℂ) ⋊ ℂ$
#indent[
  $GL(2,ℂ),SL(2,ℂ)$ are #link(<action_surjective>)[surjective action], orbit number $1$, so calculate isotropy #link(<isotropy_in_same_orbit_is_isom>)[only need to consider] one point
