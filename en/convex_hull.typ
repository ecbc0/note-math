...
#indent[
  ...
  $
  Intuition for affine combination: the simplest example is proportional points on a line determined by two points #image("../image/affine_combination_1.png", width: 40%)
  ...
  #image("../image/affine_combination_2.png", width: 30%)
...
#tag("affine_coordinate") The $t_i$ in the point $sum t_i x_i, sum t_i = 1$ can be considered as a kind of coordinate based on the points $x_i$. Called affine coordinates. *alias* barycentric coordinates #tag("barycentric_coordinate")

*Example* #tag("center_of_affine_point_set") Midpoint/centroid of affine combination $sum_(0..N) t_i x_i$ has affine coordinates $t_i ≡ 1/(N + 1)$

#tag("affine_independent") 
#indent[
...
#tag("affine_map_point_ver") *alias* #tag("simplicial_map")
...
#tag("convex_hull") $convex(x_0 ,…, x_N)$ := the affine combination $affine(x_0 ,…, x_N)$ of $x_0 ,…, x_N$ with the additional restriction $0 <= t_i <= 1$

#tag("simplex") := convex hull formed by affinely independent points

#image("../image/simplex.png", width: 70%)

#tag("extreme_point_of_convex_hull")
#indent[
...
*Prop* #tag("finite_intersect_preserve_core") $core(S_1 ∩ S_2) = core(S_1) ∩ core(S_2)$. _Proof_ $⊆$ is obvious. $⊇$ uses $λ = min(λ_1, λ_2)$

*Def* #tag("linearly_accessible") Similar to topological closure. $y in #lin_a (S) := (y in S) or (exists v in V "with" v != 0, #o_intv (y, y + v) ⊆ S)$
...
#indent[
  ...
  #image("../image/convex_open_interval_1.png")
...
*Prop* #tag("core_open_interval_core") let $y in #lin_a (S)$, let $x in core(S)$. then $#o_intv (x,y) ⊆ core(S)$

_Proof_
#indent[
  If $y in S$ then reduce to previous prop

  #image("../image/convex_open_interval_2.png")
...
#indent[
  ...
  #image("../image/convex_linearly_accessible.png")
...
*Prop* #tag("finite_intersect_preserve_convex_closed") $#lin_a (S_1) = S_1, #lin_a (S_2) = S_2 ==> #lin_a (S_1 ∩ S_2) = S_1 ∩ S_2$
...
*Prop* #tag("full_dim_iff_core_nonempty") $dim S = n <==> core(S) != ∅$
...
*Prop* #tag("convex_of_union") Let $A, B$ be convex, then $convex(A ∪ B) = {(1 - λ) a + λ b : a in A, b in B, λ in [0,1]}$
...
#indent[
  ...
  By #link(<maximal_linear_order_exists>)[], take a maximal linearly ordered chain, take the union of the components of the pairs respectively, denote the result as $(C,D)$

  It is easy to prove $C ∩ D = ∅$. We also need to prove $C ∪ D = L$

  Suppose $p in L ∖ (C ∪ D)$

  #image("/image/convex_separate_1.png", width: 80%)

  Obviously $C ⊊ convex(C ∪ p)$, by the maximality of $(C,D)$ in the partial order, $convex(C ∪ p) ∩ D != ∅$
  
  Take $d_1 in convex(C ∪ p) ∩ D$. Since $C ∩ D = ∅$, $d_1 ∉ C$. By #link(<convex_of_union>)[], $convex(C ∪ p) = {λ C + (1 - λ) p : λ in [0,1]}$, thus there exists $c in C$ such that $d_1 in intv(c,p)$
...
#indent[
  - $H$ is flat
    ...
    Suppose there exists $t > 1$ such that $z(t) ∉ H$. Assume $z(t) in core(C)$. Since $x in #lin_a (C)$, by #link(<core_open_interval_core>)[], $y in #o_intv (x,z) ⊆ core(C)$, contradicting $y in bd(C)$ 
  ...
  - $codim(H) = 1$
    ...
    #image("../image/convex_separate_2.png", width: 80%)
]

*Prop* #tag("hyperplane_separation") Hyperplane separation theorem. Let $A,B$ be convex and $(core(A), B != ∅) and (core(A) ∩ B = ∅)$, then there exists a hyperplane $H$ separating $A,B$

Note: Separation means $A ⊆ H_+, B ⊆ H_-$, not strict separation $A ⊊ H_+, B ⊊ H_-$

*Prop* $H$ separates $#lin_a (A), #lin_a (B)$

*Def* #tag("support_hyperplane") Supporting hyperplane: $S$ lies on one side of hyperplane $H$ and $bd(S) ∩ H != ∅$
...
*Def* #tag("order_of_boundary") The intersection $F = ⋂ { H "support at" x }$ of all supporting hyperplanes at $x in bd(S)$ is an affine subspace, the order $ord(x) := dim F$ is defined as its dimension

*Def* #tag("vertex") Vertex := boundary point of order $0$

*Prop* #tag("bd_convex_hull_core") let $x,y in bd(S)$. if $exists t in (0,1), (1 - t) x + t y in core(S)$, then $#o_intv (x, y) ⊆ core(S)$

*Prop* #tag("bd_convex_hull_bd") let $x,y in bd(S)$. if $exists t in (0,1), (1 - t) x + t y in bd(S)$, then $#o_intv (x, y) ⊆ bd(S)$
...
*Prop* #tag("support_hyperplane_at_core_of_convex_bd_subset") Let $B ⊆ bd(S)$ be convex, let $A$ be the affine subspace spanned by $B$, then any supporting hyperplane $H$ containing $x in core(B)$ (core in $A$) satisfies $B ⊆ H$, hence $A ⊆ H$
...
#tag("polar_hyperplane") Conversely, in a coordinate system, $(f_1 ,…, f_n) != 0$ gives the hyperplane $(f_1 ,…, f_n)^† = H = {a in V : f_1 a_1 + ⋯ + f_n a_n = 1}$
...
#tag("polar_dual") let $A ⊆ V$. $A^* := {a' in V : forall(a in A), a ⋅ a' <= 1} = ⋂_(a in A) a^†_-$
...
#tag("extreme_hyperplane") *Def* Extreme hyperplane := one that cannot be expressed as containing the region defined by the intersection of other half-spaces
...
*Def* #tag("extreme_point") An extreme point of a general convex set $S$ is defined as $x in bd(S)$ such that $S ∖ x$ is convex
...
#indent[
  ...
  By #link(<core_open_interval_core>)[], $y_1, y_2 in bd(S)$
  
  By #link(<bd_convex_hull_bd>)[], $intv(y_1, y_2) ⊆ bd(S)$
  
  By #link(<support_hyperplane_at_core_of_convex_bd_subset>)[], $ord(x) >= 1$, thus $x$ is not a vertex
]
...
_Proof_ 
#indent[
  By #link(<support_hyperplane_at_core_of_convex_bd_subset>)[], if $k < n-1$, then $x$ is not in the core of an $(n-1)$-face, so it lies in a facet of an $(n-1)$-face
...
*Prop* An affine subspace $F$ of $L$ is convex closed. _Proof_ Cases with dimension $>= n-1$ have been proven. For dimension $k < n-1$, it is the intersection of $n-k$ convex closed sets, hence by #link(<finite_intersect_preserve_convex_closed>)[], $F$ is also convex closed
...
+ $core(S^*)$ contains the origin, hence by #link(<full_dim_iff_core_nonempty>)[], $S^*$ is $n$-dimensional
...
#tag("convex_hull_decomposition") convex hull can be decomposed into simplices in the following way
...
Proceed by induction on dimension. Take a vertex $x_i$

Classify the facets into
- those containing $x_i$ 
- those not containing $x_i$

$H$ can be decomposed into the cones formed by $x_i$ and the facets $F_i$ that do not contain $x_i$
...
*Prop* #tag("opposite_facet") Let $S$ be an $n$-dimensional bounded H-polytope, and let $v$ be an extreme point of $S$. For any $x in S ∖ {v}$, the ray $f(t) = (1 - t) v + t x$ ($t >= 0$) starting from $v$ and passing through $x$ intersects $bd(S)$ at a unique point $y$ such that $t > t_y ==> f(t) ∉ S$. Then $y$ must lie on some facet that does not contain $v$.

_Proof_
#indent[
  Since $S$ is bounded and convex closed, the ray will leave $S$ for sufficiently large $t$. 
  Let $t_y = max {t >= 0 : f(t) in S}$, and let the boundary point be $y = f(t_y) in bd(S)$.
  
  Suppose $S$ is formed by the intersection of halfspaces $S = ⋂_(i = 1..m) H_(i -)$.
  
  According to a known result, the boundary equals the union of all facets: $bd(S) = ⋃_(i = 1..m) facet(S,i)$, where $facet(S,i) = H_i ∩ S$.
  
  Since $y in bd(S)$, there must exist some facets containing $y$. Let the index set of facets containing $y$ be $I(y) = {i : y in H_i}$.
  
  We proceed by contradiction. Assume that all facets containing $y$ also contain $v$, i.e.:
  $
    forall i in I(y), v in H_i
  $
  
  Since $y in H_i$ and $v in H_i$, and $H_i$ is an affine hyperplane, the entire line lies within $H_i$. Hence for all $t >= 0$, the ray $f(t) in H_i ⊆ H_(i -)$. This means the ray never violates the halfspace constraints for those indices in $I(y)$.

  For hyperplanes with $k in.not I(y)$, we have $y in S$ and $y in.not H_k$, so $y$ must lie strictly in the interior of those halfspaces, i.e.:
  $
    forall k in.not I(y), y in core(H_(k -))
  $
  
  $core(H_(k -))$ is an open set, and there are only finitely many hyperplanes not in $I(y)$. 
  This means, moving slightly forward from $y$ along the ray direction (i.e., $t = t_y + ε, ε > 0$), the ray will still remain in the interior of all $core(H_(k -))$.
  
  Now consider the point $y' = f(t_y + ε)$:
  - For $i in I(y)$, $y' in H_i ⊆ H_(i -)$ (since the whole line lies in $H_i$).
  - For $k in.not I(y)$, $y' in H_(k -)$ (because $ε$ is small enough not to exit $core(H_(k -))$).
  
  This shows $y' in ⋂_(j = 1..m) H_(j -) = S$.
  
  But this contradicts the fact that $t_y$ is the "maximum parameter" ($max t$) for which the ray remains in $S$.
  
  The contradiction shows the assumption is false. Therefore, there must exist at least one index $j in I(y)$ such that $v in.not H_j$ and $y in H_j$.  
]

*Prop* #tag("simplex_vertex_core_ray_to_facet_core") Let $Σ = convex(v, u_1, …, u_n)$ be a simplex generated by affinely independent extreme points, and let $Δ = convex(u_1, …, u_n)$ be its facet not containing $v$. If $x in core(Σ)$, then the unique intersection point $y$ of the ray $f(λ) = (1 - λ) v + λ x$ ($λ >= 0$) starting from $v$ and passing through $x$ with the hyperplane containing $Δ$ must satisfy $y in core(Δ)$.

_Proof_
#indent[
  Since the vertices $v, u_1, …, u_n$ of $Σ$ are affinely independent, any point in space can be uniquely expressed as an affine combination of these $n+1$ points. 

  In particular, because $x in core(Σ)$, $x$ has strictly positive affine coordinates:
  $
    x = c_v v + sum_(i=1)^n c_i u_i 
  $
  where $c_v > 0$, all $c_i > 0$, and $c_v + sum_(i=1)^n c_i = 1$.

  Now write the equation of the ray from $v$ through $x$:
  $
    f(λ) &= (1 - λ) v + λ x \
         &= (1 - λ) v + λ (c_v v + sum_(i=1)^n c_i u_i) \
         &= (1 - λ + λ c_v) v + sum_(i=1)^n (λ c_i) u_i
  $
  Since the affine hyperplane containing $Δ$ is spanned by $u_1, …, u_n$, the condition for the ray to intersect this hyperplane at $y$ is equivalent to: *the affine coordinate of $y = f(λ)$ with respect to $v$ equals $0$*.
  
  Set the coefficient of $v$ to $0$:
  $
    1 - λ + λ c_v = 0 \
    λ (1 - c_v) = 1 \
    λ = 1 / (1 - c_v)
  $
  Since $c_v in (0,1)$, there is a unique intersection point, and $λ > 1$ (consistent with the intuition that the ray extends outward).

  Substitute the obtained $λ$ back into $y = f(λ)$:
  $
    y = sum_(i=1)^n (frac(c_i, 1 - c_v)) u_i
  $
  
  The affine coordinates $t_i = c_i / (1 - c_v)$ of $y$ with respect to $u_1, …, u_n$ are strictly positive: because $c_i > 0$ and $1 - c_v > 0$, we have $t_i > 0$, which is equivalent to $y in core(convex(u_1, …, u_n))$, i.e., $y in core(Δ)$. 
]

*Prop* #tag("convex_hull_decomposition_to_simplices") A convex hull can be decomposed into simplices satisfying:
+ The vertices of the simplices belong to the vertices of the convex hull;
+ The interiors ($core$) of the simplices are mutually disjoint;
+ The union of the simplices is the convex hull;
+ (Simplicial complex intersection property) For any two simplices $A, B$, their intersection $A ∩ B$ is either empty or a common face of $A$ and $B$.

_Proof_
#indent[
  Proceed by mathematical induction on the dimension of the polytope $n = dim(S)$.

  To ensure that the decompositions of adjacent faces are perfectly consistent on their intersections (thus guaranteeing condition 4), we *pre-specify a global strict total order* on the vertex set $V$ of $S$ before starting the induction (e.g., assign indices to vertices $x_0 < x_1 < ⋯ < x_N$). In each recursive step below, we always select the *smallest* vertex according to this order as the apex for the cone.

  *Base case*: $n = 0$ (a point) or $n = 1$ (a line segment). Each is itself a simplex, satisfying all conditions, and the intersection property holds trivially.

  *Inductive step*: Assume the decomposition statement holds for all V-polytopes of dimension $< n$.
  
  Let $S$ be an $n$-dimensional V-polytope. Let $v in V$ be the *smallest* vertex among all vertices of $S$ according to the global total order.

  Find all facets ($n-1$ faces) of $S$ that *do not contain* $v$, denote them by $F_1, F_2 ,…, F_m$. 
  
  By the induction hypothesis, each $F_i$ is itself an $(n-1)$-dimensional V-polytope and can be decomposed into a set of $(n-1)$-dimensional simplices $T_i = {Δ_(i,1) ,…, Δ_(i,k_i)}$.
  
  For each $(n-1)$-dimensional simplex $Δ$ in each $F_i$, construct an $n$-dimensional simplex $Σ = convex(v ∪ Δ)$.
  
  Collect all such $Σ$ to form the decomposition set $T$ of $S$. We now prove that $T$ satisfies the four required conditions:

  *Condition 1 (Vertices are extreme points)*: 
  #indent[
    Clearly $v in V$, and by the induction hypothesis, the vertices of $Δ$ belong to the vertices of $F_i ⊆ V$. Hence the vertices of $Σ$ all belong to $V$.
  ]

  *Condition 3 (Union equals $S$)*: 
  #indent[
    Clearly all $Σ ⊆ S$, so $⋃ Σ ⊆ S$. 
    
    Conversely, take any $x in S$. If $x = v$, then $x$ is obviously included. If $x != v$, consider the ray $f(t) = (1 - t) v + t x$ ($t >= 0$) from $v$ through $x$. Because $S$ is bounded and convex closed, this ray must eventually exit $S$, ending at $y in bd(S)$.
    
    According to #link(<opposite_facet>)[], $y$ lies on some facet $F_i$ that does not contain $v$. Since $F_i$ is covered by its subset $T_i$, there exists $Δ in T_i$ such that $y in Δ$. 
    
    $x$ lies on the segment $intv(v, y)$, thus $x in convex(v ∪ Δ) = Σ$. Hence $T$ covers the whole $S$.
  ]

  *Condition 2 (Interiors are disjoint)*: 
  #indent[
    Let $Σ_1 = convex(v ∪ Δ_1)$ and $Σ_2 = convex(v ∪ Δ_2)$ be two distinct simplices.
    
    Suppose there exists $x in core(Σ_1) ∩ core(Σ_2)$. According to #link(<simplex_vertex_core_ray_to_facet_core>)[], the ray from $v$ through $x$ extended outward to $bd(S)$ will intersect $Δ_1$ and $Δ_2$ at points $y in core(Δ_1)$ and $y in core(Δ_2)$ respectively.
    
    - If $Δ_1, Δ_2$ belong to the same facet $F_i$, then by the induction hypothesis, their interiors in $F_i$ intersect, implying $Δ_1 = Δ_2$ and hence $Σ_1 = Σ_2$, a contradiction.
    - If $Δ_1 ⊆ F_i, Δ_2 ⊆ F_j$ with $i != j$, then $y in F_i ∩ F_j$, which belongs to the $(n-2)$-dimensional boundary and cannot lie in the $(n-1)$-dimensional interior $core(Δ)$ of either, a contradiction.
    
    Hence $core(Σ_1) ∩ core(Σ_2) = ∅$.
  ]

  *Condition 4 (Simplicial complex intersection property)*:
  #indent[
    Let $Σ_1 = convex(v ∪ Δ_1), Σ_2 = convex(v ∪ Δ_2)$. Consider any point $x in Σ_1 ∩ Σ_2$. Let the ray from $v$ through $x$ extended outward to $bd(S)$ intersect $Δ_1$ and $Δ_2$ at points $y$ and $y$ respectively. 

    *(Key consistency)*: If $Δ_1$ and $Δ_2$ belong to different facets $F_i$ and $F_j$, they intersect at $F_i ∩ F_j$. Because we have consistently used the *same global total order on vertices* throughout the induction, the simplicial decompositions of $F_i$ and $F_j$ on their common intersection $F_i ∩ F_j$ are *identical*.
    
    Therefore, by the induction hypothesis, on this lower-dimensional face, $Δ_1 ∩ Δ_2$ is either empty or a common face $f$ of $Δ_1$ and $Δ_2$.
    - If empty, no such $x$ exists, so $Σ_1 ∩ Σ_2 = ∅$. 
    - If they intersect in a common face $f$, then $y in f$, implying $x in convex(v ∪ f)$.
    
    Since $f$ is a common face of $Δ_1$ and $Δ_2$, the cone $convex(v ∪ f)$ generated by adjoining the apex $v$ is naturally a common face of $Σ_1$ and $Σ_2$.
  ]
]
...
_Proof_ (from LLM)
#indent[
  Let $A, B$ be two bounded polytopes (e.g., simplices). Polytopes are usually closed sets; their pure set difference $A ∖ B$ may not have a closed boundary, so one typically works with the closure of the set difference, i.e., $A ∖ core(B)$

  Since we have proven the equivalence of V-polytopes and H-polytopes, we can view the subtrahend $B$ as an H-polytope. 
  Let $B = ⋂_(i = 1..m) H_(i -)$. \
  Then the interior of $B$ is $core(B) = ⋂_(i = 1..m) core(H_(i -))$.

  The complement of the interior of $B$ is:
  $
    core(B)^∁ = ⋃_(i = 1..m) core(H_(i -))^∁ = ⋃_(i = 1..m) H_(i +)
  $
  
  Thus the set difference we want to decompose can be written as the intersection of $A$ with the union of halfspaces:
  $
    A ∖ core(B) = A ∩ (⋃_(i = 1..m) H_(i +)) = ⋃_(i = 1..m) (A ∩ H_(i +))
  $
  Here, the set difference is expressed as a union of $m$ regions. However, these regions may have overlapping interiors. 

  To make the interiors of these regions disjoint, we use the *Sequential Cutting* technique to construct disjoint blocks:
  - $P_1 = A ∩ H_(1 +)$
  - $P_2 = A ∩ H_(1 -) ∩ H_(2 +)$
  - $P_3 = A ∩ H_(1 -) ∩ H_(2 -) ∩ H_(3 +)$
  - $…$
  - $P_k = A ∩ (⋂_(j = 1..k-1) H_(j -)) ∩ H_(k +)$
  
  Intuitively, this means: first cut $A$ with the first hyperplane, call the outside part $P_1$; then cut the remaining part of $A$ with the second hyperplane, call the outside part $P_2$, and so on.
]

*Prop* The set difference of simplices can be decomposed into simplices

*Prop* Let $ℝ^n$ have a positively oriented basis $e_1 ,…, e_n$. For a simplex $S$, the two orientations $O_1, O_2$ induced on any $(n-2)$-dimensional face $F$ by the two intersecting $(n-1)$-dimensional facets $F_1, F_2$ that meet at $F$ are strictly opposite (i.e., the boundary of the boundary is zero).

_Proof_
#indent[
  Let $F = F_1 ∩ F_2$ and let $F$ be $(n-2)$-dimensional.
  
  For convenience, place the local origin at the interior ($core(F)$) of $F$. Then the affine hyperplane containing $F$ can be regarded as an $(n-2)$-dimensional vector subspace, denote it by $W$. Choose an arbitrary basis $u_1 ,…, u_(n-2)$ of $W$.

  Since $F_1, F_2$ intersect at $F$ and are distinct, their containing $(n-1)$-dimensional hyperplanes $H_1, H_2$ are also distinct. 
  Choose a vector $w_1 in H_1$ with $w_1 in.not W$ such that $w_1$ points strictly from $F$ into the interior of $F_1$. 
  Similarly, choose a vector $w_2 in H_2$ with $w_2 in.not W$ such that $w_2$ points strictly from $F$ into the interior of $F_2$.
  
  $H_1$ can be represented as $W ⊕ span(w_1)$, $H_2$ as $W ⊕ span(w_2)$.
  Near $F$, the interior of simplex $S$ is precisely sandwiched between $H_1$ and $H_2$, entered by strictly positive linear combinations of $w_1$ and $w_2$. That is, for a point $x$ local to $F$:
  $
    x in core(S) <==> x = y + α w_1 + β w_2, quad " where " y in W, α > 0, β > 0
  $

  Now we derive the criteria for $O_1$ and $O_2$ according to the defined orientation rules:
  
  *1. Orientation $O_1$ induced by $F_1$*:
  #indent[
    We need a vector $v_n^((1))$ pointing from $F_1$ into the interior of $S$.
    Since $F_1$ lies in the region where $β = 0, α > 0$, increasing $β$ moves into the interior of $S$ ($β > 0$). The vector $w_2$ provides a positive $β$ component, so we can directly choose $v_n^((1)) = w_2$.
    
    Next, we need a vector $u_(n-1)^((1))$ pointing from $F$ into the interior of $F_1$. By definition of $w_1$, it perfectly satisfies this condition, so we choose $u_(n-1)^((1)) = w_1$.
    
    Thus, the basis $u_1 ,…, u_(n-2)$ is positively oriented under $O_1$ if and only if the following basis of $ℝ^n$ is positively oriented:
    $
      B_1 = (u_1 ,…, u_(n-2), u_(n-1)^((1)), v_n^((1))) = (u_1 ,…, u_(n-2), w_1, w_2)
    $
    (i.e., the determinant of the transition matrix from $e_1 ,…, e_n$ to $B_1$ is $> 0$).
  ]

  *2. Orientation $O_2$ induced by $F_2$*:
  #indent[
    We need a vector $v_n^((2))$ pointing from $F_2$ into the interior of $S$.
    Since $F_2$ lies in the region where $α = 0, β > 0$, increasing $α$ moves into the interior of $S$ ($α > 0$). The vector $w_1$ provides a positive $α$ component, so we can choose $v_n^((2)) = w_1$.
    
    We need a vector $u_(n-1)^((2))$ pointing from $F$ into the interior of $F_2$. By definition of $w_2$, we choose $u_(n-1)^((2)) = w_2$.
    
    Thus, the basis $u_1 ,…, u_(n-2)$ is positively oriented under $O_2$ if and only if the following basis of $ℝ^n$ is positively oriented:
    $
      B_2 = (u_1 ,…, u_(n-2), u_(n-1)^((2)), v_n^((2))) = (u_1 ,…, u_(n-2), w_2, w_1)
    $
  ]

  *3. Compare determinants to prove opposite*:
  #indent[
    Carefully compare the two bases $B_1$ and $B_2$ of $ℝ^n$:
    $
      B_1 &= (u_1 ,…, u_(n-2), w_1, w_2) \
      B_2 &= (u_1 ,…, u_(n-2), w_2, w_1)
    $
    Notice that $B_2$ is obtained from $B_1$ by swapping the last two columns. According to a basic property of determinants in linear algebra, swapping two columns of the transition matrix strictly changes its sign:
    $
      det(B_1) = - det(B_2)
    $
    This means, for any chosen basis $u_1 ,…, u_(n-2)$ of $F$, if it yields $det(B_1) > 0$ (i.e., positive orientation under rule $O_1$), then necessarily $det(B_2) < 0$ (negative orientation under rule $O_2$).
    
    Therefore, the induced orientations $O_1$ and $O_2$ are geometrically strictly opposite. QED.
  ]
]