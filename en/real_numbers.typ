...
#tag("Dedekind_cut") Irrational numbers $ℝ ∖ ℚ$
#indent[
...
- #tag("order_real") $ℝ$ order $x < y <==> (ℚ_(< x) ⊊ ℚ_(< y))$
...
$
  ...
  A ⋅ A' := {a ⋅ a' : (a,a') ∈ (A,A')}
$
- #tag("add_real") $ℝ$ addition. let $x,y ∈ ℝ$
#indent[
  $
...
- #tag("multiply_real") $ℝ$ multiplication. let $x,y > 0$
#indent[
  $
...
#tag("completeness_real") $ℝ$ completeness

#tag("exact_bound") Supremum Principle
#indent[
  ...
  #tag("monotone_convergence") Monotone bounded #link(<limit_sequence_real>)[convergence] _Proof_ use Supremum Principle
]
#tag("nested_closed_interval_theorem") Nested Closed Interval Theorem
#indent[
...
#let B = bf("B")
#let C = bf("C")
#tag("closed_interval_net_theorem") Closed interval #link(<net>)[net] $#B$ intersection non-empty $⋂ #B != ∅$
...
#tag("Cauchy_completeness_real") limit-distance-vanish sequence or net converges
...
#tag("uncountable_real") Real numbers are uncountable $|ℕ| < |ℝ|$
#indent[
  It has been proved that $|ℕ| < |Subset(ℕ)|$. cf. #link(<cardinal_increase>)[]
