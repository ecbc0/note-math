...
cf. #link(<massless_spinor_Lagrangian>)[Action of spinor field]
...
#indent[
  ...
  Similar to #link(<energy_momentum_tensor_KG>)[the case of scalar field], try to calculate the energy-momentum-tensor
...
#indent[
  ...
  $
  Similar to #link(<conserved_spatial_integral_energy_KG>)[the case of scalar field], time invariant $∂_0 E = 0$ by $∂^† T = ∂_(μ) T^(μ)_(ν) = 0$
...
#indent[
  ...
  - The domain differentiation part is still similar to #link(<angular_momentum_KG>)[the case of scalar field]
  #indent[
    $
      M = [x,T]
    $
  ...
  #indent[
    The δ diffeomorphism given by the Lorentz-Lie-algebra $so(1,3)$ is $1/4 [σ_(μ),σ_(ν)]_(◊) ϕ(x)$. (cf. #link(<square_root_of_Lorentz_Lie_algebra>)[])
    $
...
#indent[
  ...
  $
  #tag("current_gauge_spinor") $j = #i ϕ^† σ^◊ ϕ, ∂^† j = 0$ is called the 4-current of massless-spinor

  Similarly, for massive-spinor, the 4-current is $#i ⋅ vec(ϕ,ψ)^† mat(σ^◊;,σ) vec(ϕ,ψ)$
]
#tag("conserved_spatial_integral_charge_spinor") Fixing $ℝ^(1,3)$ coordinates, consider $j$ as a quantity integrable over $ℝ^3$
