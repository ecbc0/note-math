...
#tag("action_point_particle_non_relativity") 路径 $t ⇝ x(t)$ 的作用量 
...
$
对所有 δ diffeomorphism $X(t)$ 成立, 从而给出 Lagrange-equation (*alias* Euler--Lagrange-equation), 对于非相对论点粒子, #tag("Newton_equation") 
...
#indent[
  ...
  $
  在边界是零 + 对所有 δ diffeomorphism $X$, 积分是零 ==> #tag("point_particle_Lagrange_equation")
  $
    (∂ L)/(∂ x) - (#d)/(#d t) (∂ L)/(∂ vel(x)) = 0
  $
]
...
#indent[  
  ...
  δ 积分区域的变分 #tag("calculation_1_action_point_particle_non_relativity")
  $ 
  ...
  一般地, 改变区域 $I$ 通过 $exp(a(t))$ #link(<vector_field_as_δ_diffeomorphism>)[通过 δ diffeomorphism] $a(t)$ 给出
  $
    (#d)/(#d s) limits(integral)_(exp(s a(t)) I) #d t (f(t)) 
    
    = integral_(I) #d t ((#d)/(#d t) f(t) ⋅ a(t))
  $
  另一边, use #link(<integral_change_of_variable_formula>)[变量替换公式] 
  $ 
  ...
  是作用量对 (改变端点的) δ differentiation $(#d)/(#d s)(0) x(t + s a) = a vel(x)(t) = X(t)$ 在解 $x(t)$ 处的变分 #tag("calculation__action_point_particle_non_relativity")
  $
    (#d)/(#d s) integral_(t_0 + s a)^(t_1 + s a) #d t (L(t))
    &= integral #d t ((∂ L)/(∂ x) ⋅ X + (∂ L)/(∂ vel(x)) ⋅ vel(X)) \
  ...
  $
  recall #link(<calculation_1_action_point_particle_non_relativity>)[]
  $ 
    (#d)/(#d s) integral_(t_0 + s a)^(t_1 + s a) #d t (L(t))
...
#indent[
  ...
  $ 
    ...
    &= 0
  $
  所以类似 #link(<calculation_2_action_point_particle_non_relativity>)[能量的情况], with δ diffeomorphism $(#d)/(#d s) (x + s a) = a = X(t)$
  $
    0 = integral_(t_0)^(t_1) #d t ((#d)/(#d t) ((∂ L)/(∂ vel(x)) ⋅ X)) = (∂ L)/(∂ vel(x)) |_(t_0)^(t_1) ⋅ a
  $
  #tag("momentum_point_particle_non_relativity") 作用量 $integral #d t (1/2 m vel(x)^2)$ 的动量 
...
#indent[
  ...
  #tag("rotation_momentum_point_particle_non_relativity") 旋转动量 rotation-momentum *alias* 角动量 angular-momentum 
  ...
  $a ⋅ (b × c)$ = #link(<volume_of_parallelogram>)[平行体有向体积] span by $a,b,c$ in Euclidean $ℝ^3$
...
- 刚体
#indent[
  由 $SO(3)$ (or $SO(2)$) 参数化, 所以可以认为是 Euclidean 型流形 $SO(3)$ 上的非相对论粒子. 但 metric 的使用 or 动能的使用不是 $so(3)$ 的 #link(<Killing_form>)[], 因为 of $so(3)$, 因为对于不是均匀质量分布的球的物体, 在不同方向的旋转有不同的惯性. 转动惯量 moment of inertia i.e. metric $g$ 可能需要额外计算
