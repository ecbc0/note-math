...
#indent[
  ...
  $
  关于仿射组合的直觉, 最简单的例子是两点直线的比例点 #image("../image/affine_combination_1.png", width: 40%)
...
#tag("affine_coordinate") 点 $sum t_i x_i, sum t_i = 1$ 中的 $t_i$ 可以认为是基于点 $x_i$ 的一种坐标. 称为仿射坐标. *alias* 重心坐标 #tag("barycentric_coordinate")

*Example* #tag("center_of_affine_point_set") 仿射组合 $sum_(0..N) t_i x_i$ 的中点/重心的仿射坐标是 $t_i ≡ 1/(N + 1)$
...
#tag("convex_hull") $convex(x_0 ,…, x_N)$ := $x_0 ,…, x_N$ 的仿射组合 $affine(x_0 ,…, x_N)$ 再额外限制 $0 <= t_i <= 1$
...
*Prop* #tag("finite_intersect_preserve_core") $core(S_1 ∩ S_2) = core(S_1) ∩ core(S_2)$. _Proof_ $⊆$ 显然. $⊇$ 用 $λ = min(λ_1, λ_2)$

*Def* #tag("linearly_accessible") 类似于拓扑闭包. $y in #lin_a (S) := (y in S) or (exists v in V "with" v != 0, #o_intv (y, y + v) ⊆ S)$
...
*Prop* #tag("convex_of_union") 设 $A, B$ convex 则 $convex(A ∪ B) = {(1 - λ) a + λ b : a in A, b in B, λ in [0,1]}$
...
#indent[
  ...
  根据 #link(<maximal_linear_order_exists>)[], 取一个极大线序链, 分别对 pair 的分量取并集, 得到的东西记为 $(C,D)$
  ...
  取 $d_1 in convex(C ∪ p) ∩ D$. 由于 $C ∩ D = ∅$, 故 $d_1 ∉ C$. 根据 #link(<convex_of_union>)[], $convex(C ∪ p) = {λ C + (1 - λ) p : λ in [0,1]}$, 于是存在 $c in C$ 使得 $d_1 in intv(c,p)$
...
#indent[
  - $H$ 是平直的
    ...
    假设存在 $t > 1$ 使得 $z(t) ∉ H$. 假设 $z(t) in core(C)$. 由于 $x in #lin_a (C)$, 根据 #link(<core_open_interval_core>)[], $y in #o_intv (x,z) ⊆ core(C)$, 与 $y in bd(C)$ 矛盾 
...
*Prop* #tag("hyperplane_separation") 超平面分离定理. 设 $A,B$ convex 且 $(core(A), B != ∅) and (core(A) ∩ B = ∅)$, 则存在超平面 $H$ 分离 $A,B$

注意: 分离是 $A ⊆ H_+, B ⊆ H_-$, 而不是严格分离 $A ⊊ H_+, B ⊊ H_-$

*Prop* $H$ 分离 $#lin_a (A), #lin_a (B)$

*Def* #tag("support_hyperplane") 支撑超平面, $S$ 在超平面 $H$ 的一边且 $bd(S) ∩ H != ∅$
...
*Def* #tag("order_of_boundary") $x in bd(S)$ 的所有支撑超平面的交集 $F = ⋂ { H "support at" x }$ 是一个仿射子空间, order $ord(x) := dim F$ 定义为其维数
...
*Prop* #tag("support_hyperplane_at_core_of_convex_bd_subset") 设 $B ⊆ bd(S)$ convex, 设 $B$ 张开仿射子空间 $A$, 则任何包含 $x in core(B)$ ($A$ 里的 core) 的支撑超平面 $H$ 都有 $B ⊆ H$, 从而 $A ⊆ H$
...
#tag("polar_hyperplane") 反之, 在一个坐标系中, $(f_1 ,…, f_n) != 0$ 可以给出超平面 $(f_1 ,…, f_n)^† = H = {a in V : f_1 a_1 + ⋯ + f_n a_n = 1}$
...
#tag("extreme_hyperplane") *Def* 极超平面 := 无法包含其它超平面包围区域
...
*Def* #tag("extreme_point") 一般 convex set $S$ 的极点定义为 $x in bd(S)$ 且 $S ∖ x$ convex
...
#indent[
  ...
  根据 #link(<core_open_interval_core>)[], $y_1, y_2 in bd(S)$
  
  根据 #link(<bd_convex_hull_bd>)[], $intv(y_1, y_2) ⊆ bd(S)$
  
  根据 #link(<support_hyperplane_at_core_of_convex_bd_subset>)[], $ord(x) >= 1$, 从而 $x$ 不是顶点
]
...
_Proof_ 
#indent[
  根据 #link(<support_hyperplane_at_core_of_convex_bd_subset>)[], 如果 $k < n-1$, 则 $x$ 不会在 $n-1$ face 的 core, 所以在 $n-1$ face 的 facet
...
*Prop* $L$ 的仿射子空间 $F$ 是 convex closed 的. _Proof_ 维数 $>= n-1$ 已经证明了. 维数 $k < n-1$ 的是 $n-k$ 个 convex closed 的交集, 从而根据 #link(<finite_intersect_preserve_convex_closed>)[], $F$ 也是 convex closed
...
+ $core(S^*)$ 包含原点, 从而根据 #link(<full_dim_iff_core_nonempty>)[], $S^*$ 是 $n$ 维的
...
#tag("convex_hull_decomposition") convex hull 可以用这种方式分解到 simplex
...
*Prop* #tag("opposite_facet") 设 $S$ 是 $n$ 维有界 H 多面体, $v$ 是 $S$ 的极点. 对任意 $x in S ∖ {v}$, 从 $v$ 出发经过 $x$ 的射线 $f(t) = (1 - t) v + t x (t >= 0)$ 与 $bd(S)$ 相交于唯一一点 $y$ 使得 $t > t_y ==> f(t) ∉ S$. 则 $y$ 必然落在某个不包含 $v$ 的 facet 上.

_Proof_
#indent[
  由于 $S$ 是有界的且 convex closed, 该射线在 $t$ 足够大时必将离开 $S$. 
  令 $t_y = max {t >= 0 : f(t) in S}$, 设该边界点为 $y = f(t_y) in bd(S)$.
  
  假设 $S$ 由极超平面包围而成 $S = ⋂_(i = 1..m) H_(i -)$.
  
  根据已证的结论, 边界等于所有 facet 的并集: $bd(S) = ⋃_(i = 1..m) facet(S,i)$, 其中 $facet(S,i) = H_i ∩ S$.
  
  由于 $y in bd(S)$, 必定存在一些 facet 包含 $y$. 令包含 $y$ 的 facet 的下标集合为 $I(y) = {i : y in H_i}$.
  
  我们使用反证法. 假设所有包含 $y$ 的 facet 都包含 $v$, 即:
  $
    forall i in I(y), v in H_i
  $
  
  由于 $y in H_i$ 且 $v in H_i$, 而 $H_i$ 是仿射超平面, 故整条直线都在 $H_i$ 内. 从而对所有 $t >= 0$, 射线 $f(t) in H_i ⊆ H_(i -)$. 这意味着射线在下标属于 $I(y)$ 的这些半空间约束上, 永远不会越界.

  对于 $k in.not I(y)$ 的超平面, $y in S$ 且 $y in.not H_k$, $y$ 必定严格在这些半空间的内部, 即:
  $
    forall k in.not I(y), y in core(H_(k -))
  $
  
  $core(H_(k -))$ 是开集, 且不属于 $I(y)$ 的超平面只有有限个. 
  这意味着, 从 $y$ 开始沿着射线的方向稍微往前走一小段距离 (即 $t = t_y + ε, ε > 0$), 射线仍然会停留在所有 $core(H_(k -))$ 的内部.
  
  此时考察点 $y' = f(t_y + ε)$:
  - 对于 $i in I(y)$, $y' in H_i ⊆ H_(i -)$ (因为整条直线都在 $H_i$ 内).
  - 对于 $k in.not I(y)$, $y' in H_(k -)$ (因为 $ε$ 足够小, 还没穿出 $core(H_(k -))$).
  
  这说明 $y' in ⋂_(j = 1..m) H_(j -) = S$.
  
  但这与 $t_y$ 是射线停留在 $S$ 内的 "最大参数" ($max t$) 相矛盾
  
  矛盾表明, 假设不成立. 因此, $I(y)$ 中必定存在至少一个下标 $j$, 使得 $v in.not H_j$ 且 $y in H_j$.  
]

*Prop* #tag("simplex_vertex_core_ray_to_facet_core") 设 $Σ = convex(v, u_1, …, u_n)$ 是一个由仿射无关的极点生成的 simplex, $Δ = convex(u_1, …, u_n)$ 是其不包含 $v$ 的 facet. 若 $x in core(Σ)$, 则从 $v$ 出发穿过 $x$ 的射线 $f(λ) = (1 - λ) v + λ x$ ($λ >= 0$) 与 $Δ$ 所在超平面的唯一交点 $y$ 必定满足 $y in core(Δ)$.

_Proof_
#indent[
  因为 $Σ$ 的顶点 $v, u_1, …, u_n$ 是仿射无关的, 空间中任何点都可以用这 $n+1$ 个点写成唯一的仿射组合. 

  特别地, 因为 $x in core(Σ)$, $x$ 具有严格大于 $0$ 的仿射坐标:
  $
    x = c_v v + sum_(i=1)^n c_i u_i 
  $
  其中 $c_v > 0$, 所有 $c_i > 0$, 且 $c_v + sum_(i=1)^n c_i = 1$.
  ...
  $
  由于 $Δ$ 所在的仿射超平面是由 $u_1, …, u_n$ 展开的, 射线与该超平面相交于 $y$ 的等价条件是: *点 $y = f(λ)$ 的关于 $v$ 的仿射坐标等于 $0$*.
  ...
  $
    ...
    λ = 1 / (1 - c_v)
  $
  因为 $c_v in (0,1)$, 故存在唯一的交点, 并且 $λ > 1$ (这与射线向外延伸的直觉一致).
  ...
  $y$ 在 $u_1, …, u_n$ 上的仿射坐标 $t_i = c_i / (1 - c_v)$ 严格为正: 因为 $c_i > 0$ 且 $1 - c_v > 0$, 显然有 $t_i > 0$, 这等价于 $y in core(convex(u_1, …, u_n))$, 即 $y in core(Δ)$. 
]

*Prop* #tag("convex_hull_decomposition_to_simplices") convex hull 可以分解为 simplex, 满足:
+ simplex 的顶点属于 convex hull 的顶点;
+ simplex 的内部 ($core$) 互不相交;
+ simplex 的并集是该 convex hull;
+ (单纯复形相交性质) 任意两个 simplex $A, B$, 其交集 $A ∩ B$ 要么为空, 要么是 $A, B$ 的一个公共 face.

_Proof_
#indent[
  采用对多面体维数 $n = dim(S)$ 的数学归纳法.

  为了确保相邻的面的分解在它们的交集上完全一致 (从而保证条件 4), 我们在归纳开始前, *预先对 $S$ 的所有顶点集 $V$ 指定一个全局的严格全序* (例如给顶点编号 $x_0 < x_1 < ⋯ < x_N$). 在接下来的每一步递归中, 我们始终选取当前集合中*排序最小*的顶点作为发出射线的锥顶点 (Apex).

  *基础情况*: $n = 0$ (点) 或 $n = 1$ (线段), 它们本身就是 simplex, 满足所有条件, 交集性质也显然成立.

  *归纳步骤*: 假设对所有维数 $< n$ 的 V 多面体, 该剖分命题均成立.
  
  设 $S$ 是 $n$ 维的 V 多面体. 设 $v in V$ 是 $S$ 所有顶点中按全局全序排序*最小*的顶点.

  找出 $S$ 的所有*不包含* $v$ 的 facet ($n-1$ face), 记为 $F_1, F_2 ,…, F_m$. 
  
  由归纳假设, 每个 $F_i$ 本身是 $n-1$ 维的 V 多面体, 可以分解为一系列 $n-1$ 维的 simplex 的集合 $T_i = {Δ_(i,1) ,…, Δ_(i,k_i)}$.
  
  对于每个 $F_i$ 中的每个 $n-1$ 维 simplex $Δ$, 我们构造一个 $n$ 维 simplex $Σ = convex(v ∪ Δ)$.
  
  将所有这样的 $Σ$ 收集起来, 构成 $S$ 的分解集合 $T$. 下面证明 $T$ 满足要求的四个条件:

  *条件 1 (顶点属于极点)*: 
  #indent[
    显然 $v in V$, 且由归纳假设, $Δ$ 的顶点属于 $F_i$ 的顶点 $⊆ V$. 故 $Σ$ 的顶点全都属于 $V$.
  ]
  ...
  *条件 3 (并集为 $S$)*: 
  #indent[
    ...
    反之, 对任意 $x in S$. 若 $x = v$, 则显然 $x$ 被包含. 若 $x != v$, 考虑从 $v$ 出发穿过 $x$ 的射线 $f(t) = (1 - t) v + t x, t >= 0$. 因为 $S$ 有界且 convex closed, 该射线必然在某处穿出 $S$, 最后在 $y in bd(S)$.
    
    根据 #link(<opposite_facet>)[], $y$ 落在某个不包含 $v$ 的 facet $F_i$ 上. 因为 $F_i$ 被其子集 $T_i$ 覆盖, 存在 $Δ in T_i$ 使得 $y in Δ$. 
    
    $x$ 位于线段 $intv(v, y)$ 上, 从而 $x in convex(v ∪ Δ) = Σ$. 故 $T$ 覆盖了整个 $S$.
  ]

  *条件 2 (内部不相交)*: 
  #indent[
    设 $Σ_1 = convex(v ∪ Δ_1)$ 和 $Σ_2 = convex(v ∪ Δ_2)$ 是两个不同的 simplex.
    
    假设存在 $x in core(Σ_1) ∩ core(Σ_2)$. 根据 #link(<simplex_vertex_core_ray_to_facet_core>)[], 从 $v$ 出发穿过 $x$ 的射线向外延伸到 $bd(S)$ 会在 $Δ_1$ 和 $Δ_2$ 上分别产生交点 $y in core(Δ_1)$ 且 $y in core(Δ_2)$.
    
    - 若 $Δ_1, Δ_2$ 属于同一个 facet $F_i$, 根据归纳假设, 它们在 $F_i$ 中内部相交, 从而只能 $Δ_1 = Δ_2$, 从而 $Σ_1 = Σ_2$, 产生矛盾.
    - 若 $Δ_1 ⊆ F_i, Δ_2 ⊆ F_j$ ($i != j$), 则 $y in F_i ∩ F_j$, 属于 $n-2$ 维度的边界, 不可能位于 $n-1$ 维度的 $core(Δ)$ 中, 产生矛盾.
  ...
  *条件 4 (单纯复形相交性质)*:
  #indent[
    设 $Σ_1 = convex(v ∪ Δ_1), Σ_2 = convex(v ∪ Δ_2)$. 考察任一点 $x in Σ_1 ∩ Σ_2$. 设从 $v$ 出发穿过 $x$ 的射线向外延伸到 $bd(S)$ 会在 $Δ_1$ 和 $Δ_2$ 上分别产生交点 $y$. 

    *(关键一致性)*: 如果 $Δ_1$ 和 $Δ_2$ 属于不同的 facet $F_i$ 和 $F_j$, 它们会相交于 $F_i ∩ F_j$. 由于我们在归纳中统一使用了*同一套顶点的全局全序*, $F_i$ 和 $F_j$ 在交界处 $F_i ∩ F_j$ 上的单纯形分解是*完全相同*的.
    
    因此, 根据归纳假设, 在维数更低的面中, $Δ_1 ∩ Δ_2$ 要么为空, 要么是 $Δ_1$ 和 $Δ_2$ 的一个公共 face $f$.
    - 如果为空, 则这样的 $x$ 不存在, $Σ_1 ∩ Σ_2 = ∅$. 
    - 如果相交于公共 face $f$, 则 $y in f$, 从而 $x in convex(v ∪ f)$.
    
    由于 $f$ 是 $Δ_1$ 和 $Δ_2$ 的公共 face, 通过附加极点 $v$ 生成的锥 $convex(v ∪ f)$ 自然就是 $Σ_1$ 和 $Σ_2$ 的公共 face.
  ]
]
...
_Proof_ (from LLM)
#indent[
  设 $A, B$ 是两个有界多面体 (例如 simplex). 多面体通常是闭集, 它们的纯减集 $A ∖ B$ 可能没有闭合边界, 因此通常处理的是减集的闭包, 即 $A ∖ core(B)$

  由于我们已经证明了 V 多面体和 H 多面体的等价性, 我们可以将减数 $B$ 看作 H 多面体. 
  设 $B = ⋂_(i = 1..m) H_(i -)$. \
  则 $B$ 的内部 $core(B) = ⋂_(i = 1..m) core(H_(i -))$.
  ...
  于是我们要分解的减集可以写为 $A$ 与半空间并集的交集:
  $
    A ∖ core(B) = A ∩ (⋃_(i = 1..m) H_(i +)) = ⋃_(i = 1..m) (A ∩ H_(i +))
  $
  此时, 减集被表示为了 $m$ 个区域的并集. 但这些区域可能有相互重叠的内部. 
  ...
  这在直观上相当于: 先用第一个超平面把 $A$ 砍一刀, 留在外面的叫 $P_1$; 再把 $A$ 剩下的部分用第二个超平面砍一刀, 留在外面的叫 $P_2$, 依此类推
]

*Prop* simplex 的减集可以分解为 simplex

*Prop* 设 $ℝ^n$ 具有正定向基 $e_1 ,…, e_n$. 对于 simplex $S$, 其任一 $n-2$ 维 face $F$ 上由相交的两个 $n-1$ 维 facet $F_1, F_2$ 诱导出的两个定向 $O_1, O_2$ 是严格相反的 (即边界的边界为零).

_Proof_
#indent[
  设 $F = F_1 ∩ F_2$ 且 $F$ 是 $n-2$ 维的.
  
  为了方便表示, 将局部原点选在 $F$ 的内部 ($core(F)$). 此时 $F$ 所在的仿射超平面可以看作一个 $n-2$ 维的向量子空间, 记为 $W$. 取 $W$ 的任意一组基 $u_1 ,…, u_(n-2)$.

  由于 $F_1, F_2$ 相交于 $F$ 且不重合, 它们所在的 $n-1$ 维超平面 $H_1, H_2$ 也是不同的. 
  我们可以选取一个向量 $w_1 in H_1$ 且 $w_1 in.not W$, 使得 $w_1$ 从 $F$ 严格指向 $F_1$ 的内部. 
  同理, 选取向量 $w_2 in H_2$ 且 $w_2 in.not W$, 使得 $w_2$ 从 $F$ 严格指向 $F_2$ 的内部.
  
  $H_1$ 可以表示为 $W ⊕ span(w_1)$, $H_2$ 可以表示为 $W ⊕ span(w_2)$.
  在 $F$ 附近, simplex $S$ 的内部正好夹在 $H_1$ 和 $H_2$ 之间, 它可以由 $w_1$ 和 $w_2$ 的严格正线性组合进入. 即对于 $F$ 局部的点 $x$:
  $
    x in core(S) <==> x = y + α w_1 + β w_2, quad " where " y in W, α > 0, β > 0
  $

  现在我们根据定义的定向规则, 分别推导 $O_1$ 和 $O_2$ 的判定标准:
  
  *1. 由 $F_1$ 诱导定向 $O_1$*:
  #indent[
    我们需要一个从 $F_1$ 指向 $S$ 内部的向量 $v_n^((1))$.
    由于 $F_1$ 位于 $β = 0, α > 0$ 的区域, 在该区域增加 $β$ 即可进入 $S$ 内部 ($β > 0$). 而向量 $w_2$ 恰好提供了正的 $β$ 分量, 所以我们可以直接选取 $v_n^((1)) = w_2$.
    
    接着, 我们需要一个从 $F$ 指向 $F_1$ 内部的向量 $u_(n-1)^((1))$. 根据 $w_1$ 的定义, 它完美符合条件, 我们选取 $u_(n-1)^((1)) = w_1$.
    
    于是, 基 $u_1 ,…, u_(n-2)$ 在 $O_1$ 下是正定向的, 当且仅当下面这个基在 $ℝ^n$ 中正定向:
    $
      B_1 = (u_1 ,…, u_(n-2), u_(n-1)^((1)), v_n^((1))) = (u_1 ,…, u_(n-2), w_1, w_2)
    $
    (即从 $e_1 ,…, e_n$ 到 $B_1$ 的过渡矩阵的 $det > 0$).
  ]

  *2. 由 $F_2$ 诱导定向 $O_2$*:
  #indent[
    我们需要一个从 $F_2$ 指向 $S$ 内部的向量 $v_n^((2))$.
    由于 $F_2$ 位于 $α = 0, β > 0$ 的区域, 增加 $α$ 即可进入 $S$ 内部 ($α > 0$). 向量 $w_1$ 提供了正的 $α$ 分量, 所以我们可以选取 $v_n^((2)) = w_1$.
    
    我们需要一个从 $F$ 指向 $F_2$ 内部的向量 $u_(n-1)^((2))$. 根据 $w_2$ 的定义, 我们选取 $u_(n-1)^((2)) = w_2$.
    
    于是, 基 $u_1 ,…, u_(n-2)$ 在 $O_2$ 下是正定向的, 当且仅当下面这个基在 $ℝ^n$ 中正定向:
    $
  ...
  *3. 比较行列式, 证明相反*:
  #indent[
    仔细对比 $B_1$ 和 $B_2$ 这两组 $ℝ^n$ 中的基:
    $
      B_1 &= (u_1 ,…, u_(n-2), w_1, w_2) \
      B_2 &= (u_1 ,…, u_(n-2), w_2, w_1)
    $
    可以发现, $B_2$ 完全是由 $B_1$ 交换了最后两列得到的. 根据线性代数中行列式的基本性质, 交换过渡矩阵的两列会使得其行列式严格变号:
    $
      det(B_1) = - det(B_2)
    $
    这意味着, 任意选取 $F$ 的一组基 $u_1 ,…, u_(n-2)$, 如果它使得 $det(B_1) > 0$ (即在 $O_1$ 规则下为正方向), 那么必定有 $det(B_2) < 0$ (在 $O_2$ 规则下为负方向).
    
    因此, 诱导定向 $O_1$ 和 $O_2$ 在几何上严格相反. 证毕.
  ]
]