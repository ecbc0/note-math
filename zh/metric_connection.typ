...
其中 $Γ$ 是 #tag("metric_connection") *alias* #tag("Levi_Civita_connection") *alias* #tag("Christoffel_symbols")
#indent[
  $
    && v^⊺ Γ v
  ...
  $
  metric-connection 不是 tensor. metric-connection 的变换方式 #tag("connection_transformations")
  $
    Γ(y) 
...
#indent[
  ...
  - 1st. 
  #indent[
    #tag("differenial_of_metric_vs_connection") *Prop* $v^⊺ (∂(v'') g) v' = v'^⊺ g (v''^⊺ Γ v) + v'^⊺ g (v''^⊺ Γ v)$ _Proof_ 直接把 $Γ$ 的 #link(<metric_connection>)[定义] 代入式子
