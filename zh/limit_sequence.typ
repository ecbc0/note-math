...
#tag("sequence_real") 实数列 := $ℕ -> ℝ$. 通常记为 $a_n$. 根据情况, 设置从 $0$ 开始或从 $1$ 开始

#tag("limit_sequence_real") 数列 $a_n$ 极限
...
#tag("rational_dense_in_real") $ℚ$ 在 $ℝ$ 的稠密性. $forall x,y ∈ ℝ, exists m/n ∈ ℚ, x < m/n < y$

_Proof_
#indent[
  $x < y$ #link(<order_real>)[等价于]
...
#tag("geometric_series") 几何级数 $x_n = 1 + a + a^2 + ⋯ + a^n$. $0 < a < 1 ==> lim_(n ->  ∞) x_n = 1/(1 - a)$

_Proof_ $(1 - a)(1 + a + a^2 + ⋯ + a^n) = 1 - a^(n + 1)$, $x_n = (1 - a^(n + 1))/(1 - a)$

#tag("geometric_series_test") 几何级数收敛判别. let $a_n > 0$. $a_(n + 1)/a_n < q < 1 ==> lim_(n ->  ∞) a_n = 0$

_Proof_ $a_n = a_n/a_(n - 1) ⋯ a_0/a_1 a_0 ⋯  < q^n a_0$

#tag("exponential_vs_power") 指数增长快于幂. $a > 1 , p ∈ ℕ ==> lim_(n -> ∞) n^p/a^n = 0$
...
#tag("factorial_vs_exponential_1") 阶乘增长快于指数. $a ∈ ℝ ==> lim_(n -> ∞) a^n/n! = 0$
...
#tag("mean_inequality") 均值不等式 *alias* #tag("AM_GM_inequality") 
#indent[
...
#tag("best_multiplication_decomposition") 最优乘法分解 
#indent[
...
#indent[
  ...
  $
    ...
    &= (z + 1) ⋯ (z/n + 1)
  $
  用 #link(<Taylor_expansion>)[Taylor 展开] $log (1 + z/n) = z/n + O(z/n)^2 $ 
  $
  ...
  - 用阶乘函数的性质可以证明 $sum_(n = 1 .. ∞) 1/n^2 = (π^2)/6$ cf. #link(<Euler_reflection_formula>)[]. 这里只证明收敛
  ...
  - $lim_(n -> ∞) 1 + 1/2 + ⋯ + 1/n - log n = γ$ 是 #link(<Euler_constant>)[Euler gamma 常数] #tag("Euler_constant")
  #indent[
...
#tag("Euler_reflection_formula") Euler 反射公式 $1/(z! (-z)!) = product_(n = 1 .. ∞) (1 - z^2/n^2) = (sin π z)/(π z)$ or $z/((z/π)! (-z/π)!) = z product_(n = 1 .. ∞) (1 - z^2/n^2) = sin z$
#indent[  
  ...
  $product_(n = 1 .. ∞) (1 - z^2/n^2)$ (渐进地) 被 $sum_(n = 1 .. ∞) z^2/n^2$ 控制. 有限求积在展开乘法后是一个多项式 $P_N (z)$. 在 $|z| <= R$ 用 $sum_(n = 1 .. ∞) R^2/n^2$ 控制, 给出 compact 一致收敛, 可以证明序列 $P_N (z)$ 的 $k$ 阶导数收敛到 $P(z)$ 的 $k$ 阶导数, 从而可以证明序列 $P_N (z)$ 的幂函数 $z^k$ 的系数 $a_(k, N)$ 收敛到 $P(z)$ 的幂函数 $z^k$ 的系数 $a_k$
...
#indent[
  ...
  #tag("sequence_multiplication_mean_limit") 乘法平均不改变极限
...
#tag("sequence_addition_mean_limit") 加法平均 $lim_(n -> ∞) a_n = a ==> lim_(n -> ∞) 1/n (a_1 + ⋯ + a_n) = a$

#tag("harmonic_series_diverge") 调和级数发散 $lim_(n -> ∞) 1 + 1/2 + ⋯ + 1/n = +∞$

_Proof_ $a_n = 1 + 1/2 + ⋯ + 1/n$ 发散 by 它不是 #link(<Cauchy_completeness_real>)[limit-distance-vanish]. e.g.
...
#indent[
  ...
  $
    ...
    &= 1
  $
  所以 $log(frac((n/e)^n, n!))$ 可以拆解为收敛到常数的项
  $
  ...
  $
    - 1/2 log n = log n^(-1/2)
  $
  所以 $frac((n/e)^n, n!) ∼ C^(-1) n^(-1/2)$ or 
  
  $frac(n!, (n/e)^n) ∼ C n^(1/2)$ or 
  
  $n! ∼ C n^(1/2) (n/e)^n$

  我们还需要计算剩下的常数 $C = lim_(n -> ∞) frac(n!, (n/e)^n n^(1/2))$
  ...
  $
  函数 $f_n (y) = (1 + frac(y, n^(1/2)))^n e^(- n^(1/2) y)$ 在 $y <= 0, y >= - n^(1/2)$ 分别在 $n -> ∞$ 时单调收敛到 $e^(-1/2 y^2)$, 因为:
  $
  ...
  所以 $n! ∼ (2 π)^(1/2) n^(1/2) (n/e)^n$

  $π$ 的出现的讨论, 也见 #link(<why_pi_in_Gaussian_integral>)[]
]