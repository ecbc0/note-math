...
#tag("shear-transformation") 平行体分解成 $n!$ 块 simplex 之后, 选取一块 simplex, 切割, 平移, 形成新平行体, 有相同体积. 称为剪切变换. e.g. $t_1 (v_1 + v_2) + t_2 v_2 + ⋯ +  t_n v_n$. 如下图所示
...
#tag("try-to-define-volume-of-low-dim") 把 $k$ 子空间看作流形, 例如在上面选取一个 $k$ 基建立坐标系, 它们就有自己的体积. 但是 $ℝ^n$ 有很多 $k$ 子空间. 如果我们只需要考虑一个 $k$ 子空间或者 $k$ 子流形, 那么问题就到此为止. 但是我们想要同时对所有 $k$ 子空间定义体积, 同时在每个 $k$ 子空间选择一个 $k$ 基建立坐标系来定义这个 $k$ 子空间的 $k$ 体积, 怎么样的选取才是好的?
...
2. 选取一个非退化二次型
#indent[
  #link(<tensor-induced-quadratic-form>)[导出] 交错空间的二次型 $⟨ v_1 ∧ ⋯ ∧ v_k ⟩^2  = det ⟨ v_i , v_j ⟩$
...
#tag("convex-hull-decomposition") convex hull 以这种方式分解到 simplex
