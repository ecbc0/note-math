...
for #link(<Klein__Gordon_Lagrangian>)[], 恢复 Planck 常数 $ℏ$, 光速 $c$, 时间 $x_0 = c t$
$
...
非相对论纯量场 Lagrangian, *alias* Schrodinger-Lagrangian #tag("Schrodinger_Lagrangian")
$
...
非相对论纯量场方程, *alias* Schrodinger-equation #tag("Schrodinger_equation")
$
...
*Eaxmple* #tag("Schrodinger_eq_potential_example") 
- $V = k r^2$ 
...
$
类似于相对论纯量场的情况, energy-momentum tensor #tag("energy_momentum_tensor_Schrodinger")
$
  T = T^(μ)_(ν) = cases(
...
旋转角动量是 #tag("angular_momentum_Schrodinger")
$
  M^k_(i j) = [x_i,T^k_j]
$
Schrodinger 场的规范电流
...
整个电流, 除以 $#i ℏ$ 之后, 是 $(ψ^* ψ, - (#i ℏ)/(2m) (ψ^* ∂_x ψ - ψ ∂_x ψ^*)$ #tag("current_gauge_Schrodinger")

Schrodinger 场规范电流的时间分量是实数, 空间分量是纯虚数

认为 $j$ 是可 $ℝ^3$ 积分的量

对时间 $t$ 不变 $∂_0 integral_(ℝ^3) #d x ( j^0 ) = 0$ by $∂^† T = 0$ #tag("conserved_spatial_integral_charge_Schrodinger")
$ 
...
#tag("motivation_of_quantization")
#indent[
  大部分对量子化的处理都会假设
  ...
  注意经典能量和量子能量的不同, 类似平均值和方差的不同. 例如, $⟨ #p ⟩^2 != ⟨ #p^2 ⟩$. 可以考虑标准差 $Δ #p = (⟨ #p^2 ⟩ - ⟨ #p ⟩^2)^(1/2)$. 有不确定性原理 $1/2 ℏ = 1/2 |#i [#x,#p]| <= Δ #x ⋅ Δ #p$. 取得等式 <==> #link(<harmonic_oscillator_ground_state>)[]
...
#tag("quamtum_operator_motivation") 
#indent[
