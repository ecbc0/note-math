...
$
由于 metric 在光锥上是零, 很多分析不能做. 而且光锥上的射线进行 quotient, 也对应 #link(<metric_cannot_distinguish_colinear_light_like>)[metric 完全不能区分共线的类光]
...
#tag("complex_struct_of_4d_projective_lightcone") 4d projective-lightcone 的复结构 (图)
  ...
  - 双曲型 $#projective-cone (1,3) ∖ #projective-cone (1,2) ≃ ℍ𝕪^2$ \ 
    ...
    是否有 $ℂℙ^1$ 类似物? 但是 $ℍ𝕪^2$ 是 Euclidean 型流形, 不适合分裂复数 $ℂ_"split"$ 的 $1,1$ signature, 而且 #link(<stereographic_projective_hyperbolic>)[双曲球极投影] 似乎挺复杂的 \
...
#indent[
  ...
  - $𝕊^2 ≃ ℂℙ^1$
  #indent[
    #link(<stereographic_projection>)[球极投影] transition-function 是二次型反演
    ...
    更直接的 $ℂℙ^1 -> 𝕊^2$ 坐标之间的映射, cf. #link(<Hopf_bundle>)[] 
  ]
]
...
- $SO(1,3)$ 作用于 $ℂℙ^1$ in coordinate 可以表示为 $SL(2,ℂ)$ #link(<linear_fractional>)[] 
...
#indent[
  ...
  #indent[
    ...
    - $A = mat(e^(1/2 φ) ; , e^(-1/2 φ))$ act on $ℂ^2$, 生成元 $1/2 mat(1;,-1) ∈ #i su(2)$
  ]
  ...
  可以证明 $sl(2,ℂ) = su(2) + #i su(2)$, 可以证明 $sl(2,ℂ) ≃ so(1,3)$
  ...
  - from $SO(1,3)$ to $SL(2,ℂ)$. 从后面的 $SL(2,ℂ)$ to $SO(1,3)$ 中反解出来. 或者用 $SO(1,3)$ #link(<polar_decomposition_of_Lorentz_group>)[Polar decomposition] to rotation boost + #link(<Euler_angle_Lorentz_group>)[Euler 角] 

  - from $A = mat(a,b;c,d) in SL(2,ℂ)$ to $Λ in SO(1,3)$, where $a d - b c = 1$ 
  #indent[
    使用 #link(<spacetime_momentum_spinor_representation>)[] 直接计算 $A #spin($p$) A^† in ℝ^(1,3)$  
    $ 
...
#tag("isotropy_on_projective_lightcone") *Prop* $SL(2,ℂ)$ 作用在 projective-lightcone $ℂℙ^1$, #link(<isotropy>)[] 类似于 $GL(1,ℂ) ⋊ ℂ$
#indent[
  $GL(2,ℂ),SL(2,ℂ)$ 是 #link(<action_surjective>)[满射作用], orbit 数 $1$, 所以计算 isotropy #link(<isotropy_in_same_orbit_is_isom>)[只需要考虑] 一点
