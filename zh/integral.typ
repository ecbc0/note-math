...
#tag("integral_on_form") 对于 $n$ 区域的积分, 函数对体积积分等价于对 $n$ form 积分. 如果认为是对 $n$ form 积分, 则积分是 $GL$ invariant 的
...
#indent[
  ...
  将换坐标映射的微分 $#d f$ at 每个 simplex 中心 as 仿射映射作用于定义域 simplex 得到值域 simplex 用于近似, 对有界区域对 #link(<mean_value_theorem_analytic>)[微分中值定理] (高阶) 近似进行 compact 一致控制, 然后取分割极限 (@ref-12, p.92--99)
  
  然后无界区域是来自有界区域的可数逼近, $sum_(i = 1 .. ∞) ε_i < ε$ 技术

  如果认为是对 $n$ form 积分, 则积分变量替换等价于 $n$ form 积分 (cf. #link(<integral_on_form>)[]) is 微分同胚 invariant
]
#tag("integral_on_manfold") *Question* 
#indent[
  根据变量替换公式, 流形上的坐标里的对 $n$ form 的积分 invariant (cf. #link(<integral_on_form>)[])
