...
#tag("convergence_radius_1d") 收敛半径
#indent[
  (cf. #link(<limsup>)[])
  $ 
...
#indent[
  *Prop* $|v| < R$ ==> $a_n v^n$ 绝对收敛
  ...
  #indent()[  
    ...
    use #link(<geometric_series_test>)[几何级数判别] and $|a_n|^(1/n) |v| <= (|v|)/R < 1$
...
#indent[
  use $|v| <= r < R$. use #link(<geometric_serise_test>)[几何级数控制]
...
将多项式的 #link(<change_base_point_polynomial>)[] 推广到级数
...
将多项式函数 #link(<polynomial_function>)[] 推广到幂级数 $sum A_n (v^n)$
...
_Proof_ (of 发散) 
#indent[
  use #link(<linear_map_induced_norm>)[] $|A_n|$, 存在 $v_n in 𝕂ℙ^(d-1)$ 使 $|A_n ((v_n)^n)| ≈ |A_n|$
...
for $f(x+v) = sum A_n (v^n)$, $n$ 阶 #link(<difference_polynomial>)[差分] 给出 
...
#tag("polynomial_expansion") 多项式展开 $ f(x+v) ∼ sum 1/(n!) #d^n f(x)(v^n) $ *alias* 幂级数, Taylor 展开, Taylor 级数

#tag("polynomial_approximation") 多项式逼近 $ f(x+v) ∼ sum_(n=0..N) 1/(n!) #d^n f(x)(v^n) + o(v^N) $ *alias* Taylor 展开, Taylor 逼近, Taylor 多项式 #tag("Taylor_expansion") #tag("Taylor_approximation") #tag("Taylor_polynomial")
...
$
  ...
  &= (∂^2 f)/(∂ v_2 ∂ v_1) (x)
$
#link(<successive_difference>)[逐次差分] 不依赖于顺序 + 极限交换 ==> 方向导数交换 $(∂^2 f)/(∂ v_1 ∂ v_2) = (∂^2 f)/(∂ v_2 ∂ v_1)$

#tag("successive_derivative") 逐次微商   
  $ 
    (∂^n f)/(∂ v_1 ⋯ ∂ v_n) (x) 
...
==> 幂级数的偏导数表示 (also cf. #link(<multi_combination>)[])
$ 
  f(x+v) 
  &= 
    sum_n 1/(n!) 
...
if use range space 坐标 $f = vec(f_1 , dots.v , f_(d'))$ 那么一阶微分 $#d f$ 表示为 Jacobi 矩阵 #tag("Jacobi_matrix")
$ 
  #d f = mat(
    (∂ f_1)/(∂ x_1) , ... , (∂ f_1)/(∂ x_d) ;
    ⋮,,⋮;
...
$ 
#tag("differential_function") 微分函数
  $
    mat( delim: #none ,
      𝕂^d &⟶ Lin(⊙^n 𝕂^d -> 𝕂^(d')) ;
      x &⟿ #d^n f(x)
...
#tag("mean_value_theorem_analytic_1d") 微分中值定理
...
#tag("fundamental_theorem_of_calculus") 微积分基本定理 
...
#indent[
  ...
  #indent[
    ...
    by 微积分基本定理 and #link(<chain_rule_1d>)[] and $(#d)/(#d t) (x + t v) = v$
...
#tag("convergence_domain") 在一点的收敛域 := ${v in 𝕂^d : lim_(N -> ∞) sum_(n=0)^N A_n (v^n) "converge"}$

计算幂级数的切换基点后的系数使用了求和的交换

for 多项式, 求和有限, 求和顺序交换, 从而切换基点良定义 #link(<change_base_point_polynomial>)[]

但是, 无限求和的极限, 如果不是绝对收敛, 并不总是兼容于求和顺序改变 #link(<series_rearrangement>)[]
...
#tag("analytic_function") 解析函数 := $f$ 定义域每一点 $x$ 附近, $f$ 都可以由 $x$ 上的幂级数来定义 $f(x + v) = sum_(n = 0..∞) A_n v^n$. 此时 $A_n = 1/n! #d^n f (x)$

#tag("analytic_isomorphism") 解析同胚 := $f : D <-> D'$
  - $f$ 双射
  - $f,f^(-1)$ 解析函数
...
#tag("homotopy_analytic") 解析 #link(<homotopy>)[同伦]

#tag("power_series_analytic_equivalent") 解析等价的幂级数 := 两个幂级数来自同一个解析函数在不同点的幂级数展开. 这等价于所有可能的解析延拓? (Riemann Surface?)

#tag("power_series_analytic_homotopy_equivalent") 解析同伦等价的幂级数 := 两个幂级数来自同一个解析函数同伦类在不同点的幂级数展开