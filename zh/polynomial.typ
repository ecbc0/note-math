...
#tag("polynomial_function_1d") 多项式函数是幂函数的有限线性组合. (仿射) 基点 $x$, (向量) 偏移 $v$
$
  f(x+v) 
  &= a_0 + a_1 v + ⋯ + a_n v^n \
  &= sum_(k = 0 .. n) a_k v^k \
...
#tag("change_base_point_polynomial") 切换基点 $x ⇝ x + Δ = y$
$
...
$
_Proof_ $f(y + v) = f(x + (Δ + v))$ 展开计算, 对比 $v^p$ 系数得到 $a_p (y)$ 的表示. 利用求和的交换来收集 $v$ 幂函数项
...
从 多项式作为有限线性组合 推广到 可数无限线性组合, 称为函数的 $ℕ$ 指数幂级数
$
  f(x+v) &= lim_(n -> ∞) a_0 + a_1 v + ⋯ + a_n v^n \
  f(x+v) &≈ a_0 + a_1 v + ⋯ + a_n v^n
$
有些函数的定义并不直接来自 $ℕ$ 指数幂级数. *Example* $1/x , 1/z$ 可以直接通过除法定义, 不需要用幂级数去定义

除了 $ℕ$ 作为可数无限数据, 还可以用 $ℤ, ℚ$

$ℕ$ 指数幂函数 $v^k$ 改为 $ℚ$ 指数幂函数 $v^(p/q)$, 会出现新的需要注意的地方
  ...
  - $p/q ∉ ℕ$ 时, 多次导数不会中断 $forall n, (v^(p/q) ⇝ (#d^n )/(#d v^n) v^(p/q) = p/q ⋯ (p/q - n + 1) v^(p/q - n) != 0)$
...
#tag("polynomial_function") 使用值域 $ℝ^d'$ 和多重线性函数 $a_k ∈ Lin((ℝ^d)^(⊗ k) → ℝ^d')$. 基点 $x$, 偏移 $v$, 定义多项式函数 
$
  f(x+v) 
  &= a_0 + a_1 v + ⋯ + a_n v^(⊗ n) \
  &= sum_(0 .. n) a_k v^(⊗ k) \
...
推广到 $ℂ$ 是简单的. $ℍ,𝕆$ 的情况, 由于非交换和非结合, 会很麻烦

可能有不同的张量 $a_k, a'_k$ 给出相同的多项式, 但是一个多项式对应唯一的对称张量 $b_k$

改变记号
- $v^(⊙ k) ⇝ v^k$ #tag("power_tensor")
- $v ⊙ w ⇝ v w$

可以通过差分 (difference) 技术, 从 $n$ 阶单项式 or 幂张量 $v^n$ 恢复到 $n$ 阶对称张量 $v_1 ⋯ v_n$
...
#tag("difference_symmetric_tensor") 对称张量 $n$ 阶差分
$
...
#tag("successive_difference") $n$ 阶差分可以写为 $n$ 次的一阶差分
$
...
#indent[
  ...
  $
  $μ : {1 ,…, m} -> I ⊆ {1 ,…, n}$ 函数可以看作是 $μ : {1 ,…, m} -> {1 ,…, n}$ 函数. 于是
  $
  ...
  $
  *Prop* 对任何非空有限集 $X$, $sum_(A ⊆ X) (-1)^(|A|) = 0$

  _Proof_
  #indent[
    全部 $A ⊆ X$ 可根据 $|A| in {0 ,…, |X|}$ 分类, 每个 $|A|$ 对应 $binom(|X|,|A|)$ 种选择 (#link(<combination>)[]). 于是
  ...
  有双射 $map(Subset {1 ,…, n}, X(μ), I, A(I, μ))$
  $
    (-1)^(|I|) 
    &= (-1)^(|I ∖ μ{1 ,…, m}| + |μ{1 ,…, m}|) \
    &= (-1)^(|A(I,μ)|) ⋅ (-1)^(|μ{1 ,…, m}|)
  ...
  最后的条件
  - $m < n$ 时对所有 $μ$ 一定成立
  - $m = n$ 时, 只有 $μ$ 是双射时才不成立 i.e. $μ$ 是所有 $n$ 阶置换, 此时 
    - $|μ{1 ,…, m}| = n$
  ...
  剩下 $sum_(μ in "permu"(n)) v_1 ⋯ v_n = |"permu"(n)| ⋅ v_1 ⋯ v_n$
  
  $n$ 阶置换有 $n!$ 个

  所以结果是 $n! ⋅ v_1 ⋯ v_n$
]
*对称* 多重线性函数 $a_m$ 的 *对称性* 使得 #link(<difference_symmetric_tensor>)[差分] 的性质能够被继承

#tag("difference_polynomial") $f(x + v) = a_n v^n$ 的 $n$ 阶差分是 $n! ⋅ a_n (v_1 ⋯ v_n)$
...
_Proof_ 设多项式 $f$ 有两个对称多重线性函数表示 $b_k, b'_k$
$
  f(x + v) = sum_(k = 0..n) b_k v^k = sum_(k = 0..n) b'_k v^k
$
先 $n$ 差分得到相同 $b_n = b'_n$. 同时移除 $n$ 阶项后, 是阶数 $< n$ 的多项式和它的两个对称多重线性表示 $b_k, b'_k$. 对 $< n$ 归纳得到结论
