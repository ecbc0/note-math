...
#tag("signature_of_2d_subspace_of_spacetime") *Prop* Minkowski $(1,n)$ 在 $dim = 2$ 的 $span(v,w)$ 的可能 signature 是 
...
- 与自身共线 $k v$ 之外的 light-like #tag("metric_cannot_distinguish_colinear_light_like")
...
#tag("simultaneity_relativity") 相对论同时性
#indent[
  根据正交基延拓, 可以证明
...
#link(<quadratic_form_inequality_Euclidean>)[内积不等式] $|⟨ v,w ⟩|^2 <= abs(v) abs(w)$ ==> #link(<triangle_inequality_Euclidean>)[三角不等式] $|v+w| <= |v|+|w|$
...
#tag("quadratic_form_inequality_Minkowski") 内积不等式
- $⟨ v ⟩^2 ⟨ w ⟩^2 < ⟨ v,w ⟩^2$ \
...
#tag("triangel_inequality_Minkowski") $1,1$ signature 三角不等式
#indent[
  $⟨ v + w ⟩^2 = ⟨ v ⟩^2 + 2 ⟨ v,w ⟩ + ⟨ w ⟩^2$
...
#tag("polor_coordinate_hyperbolic") 双曲极坐标 $ℝ^(1,1)(time,future) ≃ ℝ(>=0) × ℍ𝕪$ 
$
  mat(delim: #none,
...
$
#tag("hyperbolic_cosine_formula") 双曲余弦公式
#indent[
  let $v,w in ℍ𝕪$
...
#tag("stereographic_projective_hyperbolic") time-like 双曲面 $ℍ𝕪^n (time)$ 考虑球极投影, 两个基点分别在两枝双曲面上, 且投影在光锥方向 (下图的交叉形状) 形成分隔的奇点
