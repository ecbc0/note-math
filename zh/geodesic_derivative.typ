...
#indent[
  ...
  然而, #link(<difference_polynomial>)[difference] 操作不线性兼容于一般 diffeomorphism 的换坐标

  但是在 metric-manifold, 有特殊的坐标 --- #link(<geodesic_coordinate>)[测地线坐标]. $p$ 的不同测地线坐标的变换方式是 $SO$, 是线性的
]
#tag("geodesic_derivative") 测地线导数 *alias* #tag("Levi_Civita_derivative") Levi-Civita 导数 :=
#indent[
  在 $p$ 点测地线坐标, 在 $p$ 点的导数, $∇_X Y := (∂ Y)/(∂ X)$

  也可以对 #link(<tensor>)[] 场求导数 $∇_X T$. 根据张量结构带有的纯量乘法, 计算可以使用 #link(<Leibniz_law>)[product-rule] *Example* $∇_X (Y ⊗ Z) = (∇_X Y) ⊗ Z + Y ⊗ (∇_X Z)$
]
...
#indent[
  ...
  $
  使用 #link(<connection_transformation>)[联络的变换] 
  $
    0 &= Γ(y) \
  ...
  如果只看线性兼容, 那么有很多 #link(<principal_bundle_connection>)[线性 connection], 重合于 geodesic-derivative 的是 metric-connection
]
#tag("geodesic_derivative_of_co_vector") *Prop* 对于 co-vector 场 
$ 
  ∇ = ∂ - Gamma 
$
_Proof_ 
