...
#indent[
  ...
  $and,or,not$ 是 "冗余" 的, e.g. 逻辑等价地, 我们可以用 $not, and$ 表示 $or$
  
  电路 gate 实现是用 `not,nand`, 也即 $not, (not and)$ 作为开始. 但我觉得保持 $and, or$ 的对称性是有用的, 无论是认知上, 还是 $and, or$ 对于 $not$ 的对偶上
]
...
#indent[ 
  ...
  - ${0,1}^n$ 输入的构造
  #indent[
    根据上图, 使用 $n$ 条电路 $A_0 ,…, A_(n - 1)$ + $n$ 个 `not` gate + $2^n$ 个 `and` gate $Y_0 ,…, Y_(2^n - 1)$, 共 $2^n$ 种输入可能性
    
    有些函数只需要 $2^n$ 个输入的一部分, 不需要全部. 此时可以不接一些线
  ...
  - ${0,1}^1$ 输出的构造
  #indent[
    在 $Y_0 ,…, Y_(2^n - 1)$ 输入中, 将想要输出到 $1$ 的电线接到 `or` gate
  ...
  - ${0,1}^m$ 输出的构造. 可以认为是 $m$ 个 ${0, 1}^1$ 作为其分量
...
#indent[
  ...
  #indent[
    ...
    归纳地 ($j_1 ... j_n  =1 ... m$ 和 $j(i)$ 指取所有 ${1 ,…, n} -> {1 ,…, m}$ 函数)
  ...
  + 执行指令 
    ...
    #indent[  
      ...
      - 执行完 `add` 后, 要回到下一次循环的判断 $i < 10?$ 
        ...
        `add` 指令的下一条指令是 `jump` 指令, 在 `adress_0 + 2`, 执行结果是, 修改 `adress_of_instruction` 的值为 `adress_of_instruction - 2 = (adress_0 + 2) - 2 = adress_0`, 实现指令的跳转, 进行下一次循环的判断 $i < 10?$ 
    ]
    - $i >= 10$ 时
    #indent[  
