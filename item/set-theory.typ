...
#indent[
  ...
  #indent[
    ...
    只要使用 subtype (以及 dependent type), 集合与类型的区别就变得非常小, 例如, 对于集合 $A in Set[T]$, 对应 subtype $T(a) := { x in T | x in A }$ (或者写为 ${x: T || x in A}$, 其中 $||$ 和 $|$ 的不同用于区分类型的命题约束和集合的命题约束)

    如果证明了一个关于类型 $T$ 的定理, 有时候, 很容易看出它对某些子集成立. 但是由于将 subset 表示为类型的 subtype 加上了一个命题约束 $x in A$, 有时 $T$ 的定理又无法对某些子集成立. 总之, 对于计算机也可能要重新写一次定理. 一种减缓这种繁琐的方法是, 用 typeclass 来表示结构类型及其性质, 使得定理是基于 typeclass 的, 这样就能增加复用, 减少重复
  ]
]
...
为什么将 union 和 intersection 限制在 type/set 上? 因为, 由于没有什么理由不去定义空集族的交集并且认为它也是集合, 然而, 如果不限制在 type 上, 如果不对 $#A$ 的集合里面的元素做任何限制, 那么由于虚空论证, 空集族的交集的结果是全集 $Set$, 而全集 $Set$ 不是集合, 这破坏了集合论构造规则
