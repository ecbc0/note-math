...
对计算机来说 (cf. #raw("#link(<logic.typ>)[]")), 表示自然数的方式有
- 周期电路
- 算术元件 $+1$
- 二进制 
...
然而, 甚至存在集合的元素数量 #raw("#link(<uncountable>)[不可数]"). *Example* 
...
#tag("proposition") proposition 是特殊的 (string, bool) product struct ((字符串, 真假值) 的逻辑乘积结构) 计算机数据结构. 对于数学语言, string field 使用特殊的构造规则来限制, 此 string 限制版本称为 formula
...
#indent[
  ...
  $
  其中 "无限" 的概念有的限化是使用 #raw("#link(<declare-element-of-set>)[]") 的符号串 $a in A$
]
formula, proposition 是内存里特殊的数据结构. 可以认为 math_object, formula, proposition 是编程语言的 type

许多命题是等价的, 于是可以考虑命题的 "quotient" 类型, 但等价可能性是无限的, 是否可能从存在穷举算法, 首先要看它是否 #raw("#link(<countable>)[]") ...
...
follow #raw("#link(<bool-algebra>)[有限情况的 bool 的各种规则]"), 定义语言转换规则: let $p(a,b)$ is formula built with independent/parallel $a,b$ i.e. independent/parallel in memory read and used by specific cuntion, then
...
- 编译器读取到 formula $(forall a in A, p(a))$ is true proposition (可能来自定义 e.g. #raw("#link(<union>)[]"))) 
...
通常也被写为等价的规则 let $a' in A$, then 特殊的推导 formula $(forall a in A, p(a)) ==> p(a')$ is always true proposition. 这种形式通常被用于 #raw("#link(<proof>)[前面]") 所说的 "证明就是编译通过 $p => q$ is true proposition"
...
#indent[
  ...
  denoted by $2^A$. 根据 #raw("#link(<proposition-function>)[]") 等价于 map space $A -> {0,1}$. in finite case, number of elements $|2^A| = 2^(|A|)$

  define $∅ ⊂ A$ be true proposition
]
map space 和 subset 引入了高级别的无限

一件初看反直觉的事情是, 我们_似乎_知道所有 $S ⊂ ℕ$ 或所有 $f : ℕ -> ℕ$, 却无法数出来 #raw("#link(<cardinal-increase>)[]") #raw("#link(<uncountable>)[]"). 但是, "知道所有 $S ⊂ ℕ$" 究竟是什么意思? 事实上, 试着考虑 "一般方式之找一个 $ℕ$ 的无限子集" 这一个问题, 就会发现这不是简单的
...
#indent[
  ...
  draft of proof: 展开, 使用 parallel distributive cf. #raw("#link(<distributive-forall-exists>)[]")
]
