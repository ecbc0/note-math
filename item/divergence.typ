...
#tag("Stokes-theorem-proof") *Todo*
#indent[
  我改变主意了. 按照直观的对流形的积分和 Stokes 定理的处理, 就应该使用将流形直接分割的方法, 而不是近似分割
  
  积分中的分割直接可以用零阶可测集 (对微分同胚封闭). Stokes 定理中用于分割的区域应该是类似几何测度论的有限周长集或者 Whitney 几何积分理论的东西, 希望它们对有限的并集交集减集封闭, 对微分同胚封闭

  直观是基于, 在所有可测集中, 有一部分可测集存在有限可积分边界. 直觉上, 限制条件应该类似于, 在逼近可测集的多面体网中存在子网良好地一致控制所有射影化的或归一化 compact support form (或者其它什么更一般的东西) 在逼近多面体的边界上的积分

  证明带边流形局部是这种区域, 且这种理论的边界上的积分应该重合于流形理论中的边界上的积分
  
  Stokes 定理的证明是, 有限覆盖 form 的 compact support, 减去重复的, 分割, 然后用分割区域的 Stokes 定理, 积分在内部边界抵消, 只剩下真正的流形的边界
