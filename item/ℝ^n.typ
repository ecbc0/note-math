...
#indent[
  ...
  & "property-algebra"
  $
  *Eaxmple* #tag("complex-number") 复数. $x + y #i, x' + y' #i ∈ ℂ$
  ...
  复数的另一种动机来自多项式的因子分解. *实*多项式分解能够完全为 $(x - a)$ 或 $(x^2 + p x + q)^2$ 的形式的相乘, 而后者能在 $ℂ$ 中分解为 $(x - a) (x - a^*)$ 的形式, 特别地, $x^2 + 1 = (x - #i) (x + #i)$. 于是为了方便, 可以选择使用复数, 而且你仍然可以选择认为这只是一种代数方便, 不需要复数的几何

  但是在下面的 split complex 中却仍然无法分解 $x^2 + 1$ 为一阶多项式. 甚至 $x^2 - 1$ 有四个根, 多了两个 $1/2 (1 ± #i-split)$
...
#indent[
  ...
  *Eaxmple* #tag("octonion") 在四元数 $x_0 + x_1 #i _1 + x_2 #i _2 + x_3 #i _3$ 中使用新的虚数元 $#i _4$
  #indent[
    定义其它虚数元 
    $
      #i _5 &:= #i _1 #i _4 \
...
#indent[
  尝试给 $ℍ^n$ 定义线性代数. 我们立刻遇到问题: 由于 $ℍ$ 非交换, 纯量乘法不能随便和同一边的矩阵乘法交换. 如何定义线性结构的纯量乘法? 应该两边纯量乘法都属于一个线性结构吗? 线性映射的一种定义是作为线性空间之间的同态. 如果我们要把两边的纯量乘法都加入线性结构, 那么线性映射需要保持两边的乘法. 在 $ℍ^1$ 的情况已经看出, 这会使得线性映射到范围太过狭隘了. 所以, $ℍ^n$ "线性结构" 是定义为, 例如, 左边矩阵乘法作为线性映射, 右边纯量乘法使得线性映射是线性结构的同态.
]
