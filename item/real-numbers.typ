...
有两种加法的定义, $(a + b) + 1 = a + (b + 1)$ 或者 $(a + 1) + b = (a + b) + 1$. 在一种加法定义中通过 (在中途) 交换输入, 可以得到另一种加法定义. 证明加法交换性就是证明这两种定义对相同的输入给出相同的输出结果. 直觉上当然是结果相同的, 只是把要数数的量放到了不同的 "插槽位置", 从而有 "交换性" 

或者, 对两个位置的数数 $ℕ × ℕ$ 的函数, 一个位置 $ℕ$ 先数数后作为 "基础位置", 另一位置 $ℕ$ 再数数作为 "增值位置", 得到的结果是 "总的数数" $ℕ$. 交换律就是, 交换两个位置进行操作, 结果还是相同, 应该都是 "总的数数"
...
#indent[
  ...
  对网, 使用网的集合的上下确界, 使用 limit-distance-vanish-net 得到 net 收敛 $lim_(B in #B) sup_(a_B in B) |a_B - a| = 0$
]
