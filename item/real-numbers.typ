...
对计算机来说似乎难以表达这种直观, 但似乎所有的有限结果一定会正确. 类似于 #link(<natural-number>)[自然数中所做的], 为了让计算机能用有限的字符和内存去表达这种对所有自然数成立的性质, 需要定义它是 true proposition

通常的 "证明" 是使用最小的假设, $1$ 的结合律 $(a + b) + 1 = a + (b + 1)$ 或者说是加法的定义 $(a + 1) + b = (a + b) + 1$, 然后推出其它 
