...
#indent[
  ...
  整体 $(∆ + m^2) vec(ϕ,ψ) = 0$, square root of KG. 如果一个场满足 Dirac eq, 则它满足 KG eq. 从而 Dirac eq 可以近似到 KG eq, 或者继续近似到 Schrodinger eq. 但是注意, 此时相比于 $ℂ$ 值的情况, $ℂ^2 ⊕ ℂ^(2 ◊)$ 的不同之处在于, $so(1,3)$ or $so(3)$ 表示的角动量算子会有影响值域的自旋部分, 情况类似于 #link(<spinor-angular-momentum>)[]
...
#indent[
  ...
  *Question* 四元数 $ℍ$ 的情况如何? $ℍ$ 的情况的虚数元对称群是 $SO(3)$. 固定一个虚数元 $#i$ 之后, 是什么?
  ...
  在八元数旋量理论中, 仍然有 $𝕆^2$ 作为旋量场所在空间, $ℝ^(1, 9)$ 射影光锥是 $𝕊^8 ≃ 𝕆ℙ^1$, $SL(2, 𝕆)$ 给出 $SO(1, 9)$ 的二重覆盖, $1 + 9$ 时空对应于二阶 Hermitian 八元数矩阵 $h_2(𝕆) = mat(t - a, α; α^*, t + a)$, $det$ 对应时空 metric. 而且可以将时空, 旋量 $vec(ϕ, ψ)$, 和一个实数嵌入三阶 Hermitian 八元数矩阵 $h_3(𝕆) ≃ h_2(𝕆) + 𝕆^2 + ℝ = mat(r, 𝕆^*; 𝕆, h_2(𝕆))$ (如果我们尝试嵌入 $𝕆ℙ^1$, 则结果可能和 $h_3(𝕆)$ 的射影化 $𝕆ℙ^2$ 相关?), 保持 $h_3(𝕆)$ 的 $det$ 的群是 $E_6$, 能够良好地嵌入 $SL(2, 𝕆) ↠ SO(1, 9)$ 和旋量规范理论和 $(SU(3) × SU(2) × #U (1)) / ℤ_6$ cf. (@ref-20, th.6)
]
#tag("spin-connection")
#indent[
