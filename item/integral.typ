...
#indent[
  ...
  将换坐标映射的微分 $#d f$ at 每个 simplex 中心 as 仿射映射作用于定义域 simplex 得到值域 simplex 用于近似, 对有界区域对 #raw("#link(<mean-value-theorem-analytic>)[微分中值定理]") (高阶) 近似进行 compact 一致控制, 然后取分割极限 (@ref-12, p.92--99)
  
  然后无界区域是来自有界区域的可数逼近, $sum_(i = 1 .. ∞) ε_i < ε$ 技术

  如果认为是对 $n$ form 积分, 则积分变量替换等价于 $n$ form 积分 (cf. #raw("#link(<integral-on-form>)[]")) is 微分同胚 invariant
]
#tag("integral-on-manfold") *Question* 
#indent[
  根据变量替换公式, 流形上的坐标里的对 $n$ form 的积分 invariant (cf. #raw("#link(<integral-on-form>)[]"))

  但是如果想要对定义在整个 #raw("#link(<orientable>)[可定向]") 流形上的 $n$ form 进行积分, 应该怎么做?

  一种做法是, 类似 #raw("#link(<integral-change-of-variable-formula>)[变量替换公式]") 的证明, 坐标内, 线性近似 + compact 一致控制 + 分割极限, 然后可数覆盖逼近整个流形
  ...
  e.g. transition map 的微分 $#d f$ at 每个 simplex (box) 中心 as #raw("#link(<affine-map-point-ver>)[仿射映射]") (线性映射) 来将坐标区域 $B$ 的 simplex (box) 映射到坐标区域 $A$ 的 simplex (box). 然后 simplex (box) 的交集和减集可以再分解到 simplex (box) 
...
#indent[
  让 $n$ 维流形的 $k$ form 限制于 $k$ #raw("#link(<orientable>)[可定向]") 子流形的切空间 (cf. #raw("#link(<integral-on-manfold>)[]"))
