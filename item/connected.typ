...
*Proof* 对分解取极限得到 $Or_(i in I) X = A_i$

连通子集 := 拓扑子空间连通

*Example* $ℝ$ 连通. $ℝ$ 存在连通和不连通集. 连通集可能不是 $#T _ ℝ$ 闭集

#tag("real-connected-is-interval") $ℝ$ 的连通集就是区间 *Proof* by 区间连通 + 最优区间分解 + 最优区间分解中区间数 $> 1$ 不连通
...
*Proof* $#T _S$ 的闭集分解 $(S ∩ A) ⊔ (S ∩ A')$ 导致其中一个集合是空集

$A$ 是极限点连通集 ==> $A$ 在 $X$ 的唯一一个极限点连通分支里 

*Proof* $A$ 的点必定在 $X$ 从而在某个连通分支里
...
*Proof* 包含 $x$ 的连通集都在同一个连通分支里面. 说明 $⋃_(i in I) S_i = X$ 只有一个连通分支, 从而连通
...
*Proof* 闭集分解 $Y = A ⊔ A'$ ==> 闭集分解 $X = f^(-1)(A) ⊔ f^(-1)(A')$

==> #tag("mean-value-theorem-continuous") 连续函数介值. 连续函数 $f : X -> ℝ$ 的像 $f(X)$ 连通 #link(<real-connected-is-interval>)[从而] 是区间

如果 $Y$ 任何两点都在某个连通子集 $S$ 中, 则 $Y$ 连通. *Proof* let $Y = A ⊔ A'$ with $A,A'$ closed, 证明 $A or A' = ∅$. 或者 $Y = ⋃_(y in Y) S(y_0,y)$ and 有共同点 $y_0$ 的连通集 $S(y_0,y)$ 的并集是连通的
...
*Proof* 使用 #link(<dependent-distributive>)[] $product_(i in I) ⨆_(j in J) A_(i,j(i)) = ⨆_(j in J) product_(i in I) A_(i,j(i))$ and 连通的 product 连通, 所以 $product_(i in I) A_(i, j(i))$ 连通, 从而已经不能再分解
