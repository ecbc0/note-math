...
#indent[
  用 parser 验证 token 流语言正确性, 遍历所有语言规则, 或使用生成的有限状态机
    
  parser 的运行需要生成内存中大型复杂的数据结构 (一些形象的描述: 表格, 结点, 树)

  如果数据流是高级编程语言源代码, 还可以根据这个数据结构生成指令流. 此时称为编译 (compile), 但 parse 和 compile 经常混用
...
计算机, 除了周期电路高速的优势, 其它优势有, 内存记忆的容量 (1 GB = 2^33 bit = 8589934592 bit) 和持续性 (通电时间) 
