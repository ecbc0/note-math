...
- $SO(1,3) ≃ SL(2,ℂ)/ℤ_2$ #raw("#link(<Lorentz-group-spinor-representation>)[作用在]") $ℂℙ^1$ 提升到 $SL(2,ℂ)$ act on $ℂ^2$
#indent[
...
#indent[
  ...
  其中 $f(A)$ 是 #raw("#link(<Lorentz-group-spinor-representation>)[]")
  
  $f(#p) = p$ 是 #raw("#link(<spacetime-momentum-spinor-representation>)[]") 
...
*Prop* 将 #raw("#link(<spacetime-momentum-spinor-representation>)[]") 用于 $v ⋅ w^†$, $v = w$ + $ℂ$ 射影 $λ v$ 给出 projective-lightcone
$
...
#tag("parity") 
#indent[  
  parity 对应到 $(1/2,0)$ vs $(0,1/2)$ representation, or $A$ vs $A^*,(A^†)^(-1)$, cf. #raw("#link(<conjugate-representation>)[]")
...
这使得时空动量自旋表示有可能联系到经典 fermion 的概念. 旋量属于光锥射影 $ℂℙ^1$. 如果 $#p$ 在光锥上, 那么其平方 $1/2(#p^◊ #p + #p #p^◊) = g(#p,#p) 𝟙 = 0$. 看起来能联系到 Pauli 不相容的要求. 但注意, 一般地 $g(#p,#p') != 0$ 除非 $#p,#p'$ 共线 (#raw("#link(<signature-of-2d-subspace-of-spacetime>)[]")). 所以这种乘法的结果, $g(#p,#p') 𝟙 ≃ vec(g(#p,#p'),0,0,0)$, 将不会在光锥上. 或者如果你愿意地话, 将其拓展到 Clifford 代数
