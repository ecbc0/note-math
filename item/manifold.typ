...
- #raw("#link(<stereographic-projection>)[球极投影]") 
...
#tag("orientable") 可定向 := 在切丛中可以解析地定义 #raw("#link(<orientation>)[]")

等价于 $"Diff"$ 的 $det^(-1)(ℝ_(< 0)) ⊔ det^(-1)(ℝ_(> 0))$ 分解

等价于存在坐标覆盖, 每个 transition function 微分 $#d f in SO$

*Example* #raw("#link(<Mobius-strip>)[]") 不可定向
...
即使用二次型拓扑和微分定义了流形, 也仍然有很多不同的 metric. 一种性质良好的 metric 是 #raw("#link(<Einstein-metric.typ>)[]")

#tag("isometry") := diffeomorphism 保持 metric $g$. 通常也假设保持可定向流形的方向

微分同胚作用于 metric space, isometry 是这个群作用的 #raw("#link(<isotropy>)[]")
...
连续同胚但不微分同胚. *Example* 四元数 $ℍ$ 版本的 #raw("#link(<Hopf-bundle>)[]") 的多种修改给出了例子之 called exotic 7-shpere