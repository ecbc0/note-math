...
cf. #raw("#link(<massless-spinor-Lagrangian>)[旋量场的作用量]")
...
#indent[
  ...
  类似于 #raw("#link(<energy-momentum-tensor-KG>)[scalar field 的情况]"), 尝试计算 energy-momentum-tensor
...
#indent[
  ...
  $
  类似于 #raw("#link(<conserved-spatial-integral-energy-KG>)[纯量场的情况]"), time invariant $∂_0 E = 0$ by $∂^† T = ∂_(μ) T^(μ)_(ν) = 0$
...
#indent[
  ...
  - domain 微分部分仍然是类似 #raw("#link(<angular-momentum-KG>)[scalar field 的情况]")
  #indent[
    $
      M = [x,T]
    $
  ...
  - codomain 微分部分
  #indent[
    Lorentz-Lie-algebra $so(1,3)$ 给出的 δ diffeomorphism 是 $1/4 [σ_(μ),σ_(ν)]_(⬨) ϕ(x)$. (cf. #raw("#link(<square-root-of-Lorentz-Lie-algebra>)[]"))
    $
...
#indent[
  ...
  $
  #tag("current-gauge-spinor") $j = #i ϕ^† σ^⬨ ϕ, ∂^† j = 0$ 被称为 4 电流 of massless-spinor

  类似地, massive-spinor 的情况, 4 电流是 $#i ⋅ vec(ϕ,ψ)^† mat(σ^⬨;,σ) vec(ϕ,ψ)$
]
#tag("conserved-spatial-integral-charge-spinor") 固定 $ℝ^(1,3)$ 坐标, 认为 $j$ 是可 $ℝ^3$ 积分的量
