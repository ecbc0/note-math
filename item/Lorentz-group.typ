...
_Proof_ 使用 #raw("#link(<spacetime-momentum-aciton-spinor-representation>)[spinor 技术]")
#indent[
  *Prop* $SL(2,ℂ)$ 作用在 lightcone (不是 projective-lightcone), #raw("#link(<isotropy>)[]") 类似于 $#U (1) ⋊ ℂ$

  $SL(2,ℂ)$ 是 #raw("#link(<action-surjective>)[满射作用]"), orbit 数 $1$, 所以计算 isotropy #raw("#link(<isotropy-in-same-orbit-is-isom>)[只需要考虑]") 一点
  ...
  类似于 #raw("#link(<isotropy-on-projective-lightcone>)[]") 的计算, 此处将类似于 $#U (1) ⋊ ℂ$
]
