...
compact's original inspiration: The intersection of closed interval nets of $ℝ$ is non-empty #raw("#link(<closed-interval-net-theorem>)[]")

$0$ is a #raw("#link(<limit-point>)[limit point]") of $#T _ ℝ$ of $(0,1)$. The #raw("#link(<net>)[net]") $(0,1/n)$ seems to converge to $0$
...
let $#T _X$ #raw("#link(<topology>)[topological space]"). let $A subset X$

#tag("compact") $A$ compact := forall $#B$ net of $A$, $⋂_(B ∈ #B) closed(B) != ∅$

Meaning: The elements of any #raw("#link(<net>)[net]") $#B$ have a common limit point set under the topology $#T _X$. Or, after $#T _X$ closure, the net $#B$ converges to a non-empty set or the intersection is non-empty, instead of converging to the empty set (e.g., Euclidean $ℝ^d$ converges to the empty set or converges to infinity, but there are many other complex situations)

Any net can replenish all finite intersections and maintain #raw("#link(<net-same-limit>)[the same limit]"), so for compact, the equivalent description is 
...
recall #raw("#link(<closed-in-subspace>)[]"), $"closed"(A,#T _S) = S ∩ "closed"(A,#T _X)$, denoted as $S ∩ closed(A)$
...
#indent[
  ...
  The inverse image of a continuous function preserves closed $Closed(f^(-1)(A)) = f^(-1)(closed(A))$. Use the #raw("#link(<inverse-image>)[property]") of inverse images on $∩$
  $
...
#tag("quotient-topology-preserve-compact") For #raw("#link(<quotient-topology>)[]") $π : X ⇝ X/∼$, source space $X$ compact ==> quotient space $X/∼$ compact. because the quotient map $π$ is continuous, it preserves compact

#let prod = $limits(product)_(i in I) X_i$ 

#tag("product-topology-preserve-compact") #raw("#link(<product-topology>)[]") preserves compact
