...
#indent[
  $ϕ$ induces the co-vector of #raw("#link(<Hermitian-tensor>)[]")
  $
    dmat( delim: #none ,
      ⨀^(* 2) ℂ^2 , ⟶ , ℝ ;
      #p , ⟿ , ϕ^† #p ϕ
  ...
  #raw("#link(<parity>)[]") dual and $(0,1/2)$ induced action
  $
    #p ⇝ ϕ^† #p^⬨ ϕ ⇝ (A^† ϕ)^† #p^⬨ (A^† ϕ) 
    &=& ϕ^† (A #p^⬨ A^†) ϕ \
...
#indent[
  - formally corresponds momentum $p$ to gradient momentum $#i ⋅ (∂_0,-∂_1,-∂_2,-∂_3) = #i ⋅ (∂_0,∂_1,∂_2,∂_3)^⬨$, and $#p$ to #raw("#link(<spacetime-momentum-spinor-representation>)[]") 
    $
      #i ⋅ mat(
        ∂_0 + (-∂_1), -(∂_2 + ∂_3) #i ;
        -(∂_2 - ∂_3) #i, ∂_0 - (-∂_1)
...
#indent[
  ...
  Changing the gauge is not compatible with taking derivatives of tangent spaces in bundle coordinates, so an additional structure --- #raw("#link(<principal-bundle-connection>)[connection]") --- must be introduced.
  
  There are many possible connections. A good connection is one with the smallest curvature cf. #raw("#link(<electromagnetic-field>)[]")

  The $ℂ^2$ bundle in curved spacetime can be directly defined in the bundle coordinates of the $SO(1,3)$ principal bundle (orthonormal frame bundle). Using the #raw("#link(<Lorentz-group-spinor-representation>)[$SL(1,ℂ) ↠ SO(1,3)$ correspondence]"), changing $SO(1,3)$ bundle coordinates automatically corresponds to changing $SL(2,ℂ)$ bundle coordinates.

  In curved spacetime, one needs to deal with the covariant derivative of the spinor field with respect to the metric, which is derived from the #raw("#link(<metric-connection>)[]") of the tangent vector field. 
...
#tag("spin-connection")
#indent[
  The #raw("#link(<principal-bundle>)[frame bundle]") of $SO(1,3)$ derived from the tangent bundle metric and the #raw("#link(<principal-bundle-connection>)[connection]") of the $SO(1,3)$ frame bundle derived from #raw("#link(<metric-connection>)[]") behave as $Γ$ is locally type $(ℝ^(1,3))^⊺ ⊗ so(1,3)$, acting on $ℝ^(1,3)$ tangent vector fields by $∂ + Γ$

  The way to derive the spin-connection is to #raw("#link(<square-root-of-Lorentz-Lie-algebra>)[map the $so(1,3)$ part of the induced metric-connection $Γ$ to $sl(2,ℂ)$]") in the orthonormal-frame, yielding the connection of the $SL(2,ℂ)$ bundle, locally type $(ℝ^(1,3))^⊺ ⊗ ℂ^2$, acting on the spinor field $ℂ^2$ by $∂ + ω$ with $ω = ω^(μ ν) 1/4 [σ_(μ),σ_(ν)]_(⬨)$
